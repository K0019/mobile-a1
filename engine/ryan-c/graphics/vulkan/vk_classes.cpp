...
vk::VulkanContext::VulkanContext(const ContextConfig& config, void* window, void* display, VkSurfaceKHR surface) : vkSurface_(surface), config_(config)
{
...
VkPipeline vk::VulkanContext::getVkPipeline(ComputePipelineHandle handle)
{
  ...
  if (cps->pipeline_ == VK_NULL_HANDLE)
  {
    ...
    // create pipeline layout
    {
      // duplicate for MoltenVK
      const VkDescriptorSetLayout dsls[] = {vkDSL_, vkDSL_, vkDSL_, vkDSL_};
      const VkPushConstantRange range = {.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT, .offset = 0, .size = sm->pushConstantsSize,};
      const VkPipelineLayoutCreateInfo ci = {.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO, .setLayoutCount = (uint32_t)std::size(dsls), .pSetLayouts = dsls, .pushConstantRangeCount = 1, .pPushConstantRanges = &range,};
...
void vk::VulkanContext::createSurface(void* window, [[maybe_unused]] void* display)
{
#if defined(VK_USE_PLATFORM_WIN32_KHR)
  const VkWin32SurfaceCreateInfoKHR ci = {.sType = VK_STRUCTURE_TYPE_WIN32_SURFACE_CREATE_INFO_KHR, .hinstance = GetModuleHandle(nullptr), .hwnd = (HWND)window,};
