...
class LineCanvas3D
{
  public:
    ...
    void plane(const vec3& orig, const vec3& v1, const vec3& v2, int n1, int n2, float s1, float s2, const vec4& color, const vec4& outlineColor);
  ...
  private:
    ...
    static_assert(sizeof(linesBuffer_) / sizeof(linesBuffer_[0]) == sizeof(currentBufferSize_) / sizeof(currentBufferSize_[0]));
};
