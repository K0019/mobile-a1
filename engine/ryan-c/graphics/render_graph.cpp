...
{
  if (observer && std::ranges::find(m_transientObservers, observer) == m_transientObservers.end())
  {
...
{
  m_transientObservers.erase(std::remove(m_transientObservers.begin(), m_transientObservers.end(), observer), m_transientObservers.end());
}
...
RenderGraph::RenderGraph(vk::IContext& vkContext, GPUBuffers& gpu_buffers) : m_vkContext(vkContext), m_gpu_buffers(gpu_buffers)
{
...
void RenderGraph::Compile()
{
  ...
  RegisterTransientResource(RenderResources::SCENE_COLOR, vk::TextureDesc{.type = vk::TextureType::Tex2D, .format = m_vkContext.getSwapchainFormat(), .dimensions = ResourceProperties::SWAPCHAIN_RELATIVE_DIMENSIONS, .usage = vk::TextureUsageBits_Attachment | vk::TextureUsageBits_Sampled});
  ...
  while (i < m_compiledPassOrder.size())
  {
    ...
    textures.erase(std::ranges::unique(textures).begin(), textures.end());

    std::sort(buffers.begin(), buffers.end());
    buffers.erase(std::ranges::unique(buffers).begin(), buffers.end());
...
Render::internal::NameID RenderGraph::InternName(LogicalResourceName name)
{
  if (!name)
...
Render::internal::NameID RenderGraph::FindNameID(LogicalResourceName name) const
{
  if (!name)
    return internal::INVALID_NAME_ID;
...
Render::internal::NameID RenderGraph::ImportExternalTexture(LogicalResourceName name, vk::TextureHandle textureHandle, const vk::TextureDesc& desc)
{
  internal::NameID id = InternName(name);
  if (id == internal::INVALID_NAME_ID)
    return id;
...
Render::internal::NameID RenderGraph::ImportExternalBuffer(LogicalResourceName name, vk::BufferHandle bufferHandle, const vk::BufferDesc& desc)
{
  internal::NameID id = InternName(name);
  if (id == internal::INVALID_NAME_ID)
    return id;
...
void RenderGraph::Execute(vk::ICommandBuffer& vkCommandBuffer, const FrameData& frameData)
{
  ...
  // Check if we need to recompile due to swapchain resize
  vk::Dimensions currentDims = m_vkContext.getDimensions(m_vkContext.getCurrentSwapchainTexture());
  if (m_isCompiled && (currentDims.width != m_lastCompileDimensions.width || currentDims.height != m_lastCompileDimensions.height))
  {
    Invalidate();
  }
