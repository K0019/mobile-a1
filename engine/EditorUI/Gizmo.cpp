...
void Gizmo::draw(ImDrawList* /*viewport*/)
{
    ...
    //}
            ...
            // IMPORTANT: allow next item (our gizmo) to overlap the Image
ImGui::SetItemAllowOverlap();

// 2) Compute the rect of the just-drawn Image (this is the only rect that matters)
ImVec2 imgMin = ImGui::GetItemRectMin();
ImVec2 imgSize = ImGui::GetItemRectSize();

// 3) One BeginFrame per Dear ImGui frame + enable
ImGuizmo::SetImGuiContext(ImGui::GetCurrentContext());
static int s_lastFrame = -1;
...
// 4) Draw into the same window’s drawlist, and match the Image’s rect
ImGuizmo::SetDrawlist(ImGui::GetWindowDrawList());
ImGuizmo::SetRect(imgMin.x, imgMin.y, imgSize.x, imgSize.y);

// 5) View/Proj (already published earlier this frame)
glm::mat4 V(1.f), P(1.f);
bool isOrtho = false;
if (EditorCam_TryGet(V, P, isOrtho))
{
    ...
    // 6) Object/world matrix of selected entity (or identity)
    glm::mat4 M(1.f);
    ...
    ImGuizmo::MODE      md = EditorGizmo_Mode();
    
    // Optional snapping (Ctrl)
    const bool snapOn = ImGui::GetIO().KeyCtrl;
    float snapT[3] = { 1.0f, 1.0f, 1.0f };  // meters/units
    float snapS[3] = { 0.1f, 0.1f, 0.1f };  // scale step
    float snapDeg = 15.0f;                 // degrees
    const float* snapPtr = nullptr;
    if (snapOn) {
        if (op == ImGuizmo::TRANSLATE) snapPtr = snapT;
        else if (op == ImGuizmo::ROTATE)    snapPtr = &snapDeg;
        else if (op == ImGuizmo::SCALE)     snapPtr = snapS;
    }
