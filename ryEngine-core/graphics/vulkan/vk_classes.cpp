...
namespace
{
                                                     ...
                                                     const VkDebugUtilsMessengerCallbackDataEXT* cbData, void* userData)
  {
    ...
    //	}
    //}
    if (char typeName[128] = {}; sscanf(cbData->pMessage,
                                        ...
                                        errorName, &object, &handle, typeName, &messageID) >= 2)
    {
      ...
      LOG_DYNAMIC(
        ...
        "MessageID = {:#x} \n{} \n", isError ? "\nERROR:\n" : "", errorName, object,
        reinterpret_cast<uintptr_t>(handle), typeName, reinterpret_cast<uintptr_t>(messageID), message);
    }
    else
    {
    ...
    if (isError)
    {
      ...
      if (ctx->config_.shaderModuleErrorCallback != nullptr)
      {
        ...
        if (substr1 && sscanf(substr1, "Shader validation error occurred at line %d, column %d.", &line, &col) >= 1)
        {
          ...
          if (substr2 && sscanf(substr2, "Shader Module (Shader Module: %[^)])(%p)", shaderModuleDebugName,
                                &shaderModule) == 2)
          {
#else
            if (substr2 && sscanf(substr2, "Shader Module (Shader Module: %[^)])(%llu)", shaderModuleDebugName,
                                  &shaderModule) == 2) { 
  ...
  bool supportsFormat(VkPhysicalDevice physicalDevice, VkFormat format)
  {
    ...
    return properties.bufferFeatures != 0 || properties.linearTilingFeatures != 0 || properties.optimalTilingFeatures !=
      0;
  }
  ...
  bool validateImageLimits(VkImageType imageType, VkSampleCountFlagBits samples, const VkExtent3D& extent,
                           const VkPhysicalDeviceLimits& limits, vk::Result* outResult)
  {
    using vk::Result;
    ...
    if (imageType == VK_IMAGE_TYPE_2D && !VERIFY(
    ...
    if (imageType == VK_IMAGE_TYPE_3D && !VERIFY(
      extent.width <= limits.maxImageDimension3D && extent.height <= limits.maxImageDimension3D && extent.depth <=
  ...
  {
    if (!VERIFY(
      range.dimensions.width > 0 && range.dimensions.height > 0 || range.dimensions.depth > 0 || range.numLayers > 0 ||
      range.numMipLevels > 0))
    {
      return vk::Result{
        vk::Result::Code::ArgumentOutOfRange, "width, height, depth numLayers, and numMipLevels must be > 0"
      };
    }
    ...
    if (range.offset.x > static_cast<int32_t>(texWidth - range.dimensions.width) || range.offset.y > static_cast<
      int32_t>(texHeight - range.dimensions.height) || range.offset.z > static_cast<int32_t>(texDepth - range.dimensions
                                                                                                             .depth))
    {
      return vk::Result{vk::Result::Code::ArgumentOutOfRange, "range dimensions exceed texture dimensions"};
    }
  ...
  bool hasExtension(const char* ext, const std::vector<VkExtensionProperties>& props)
  {
    ...
    {
      if (strcmp(ext, p.extensionName) == 0) return true;
    }
    return false;
  }
  ...
  void transitionToColorAttachment(VkCommandBuffer buffer, vk::VulkanImage* colorTex)
  {
    ...
    colorTex->transitionLayout(buffer, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VkImageSubresourceRange{
  ...
  VkSurfaceFormatKHR chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& formats,
                                             vk::ColorSpace requestedColorSpace, bool hasSwapchainColorspaceExt)
  {
    ...
    auto isNativeSwapChainBGR = [](const std::vector<VkSurfaceFormatKHR>& formats) -> bool
    {
      for (const VkSurfaceFormatKHR& fmt : formats)
      {
        ...
        if (fmt.format == VK_FORMAT_R8G8B8A8_UNORM || fmt.format == VK_FORMAT_R8G8B8A8_SRGB || fmt.format ==
          VK_FORMAT_A2R10G10B10_UNORM_PACK32)
        {
          return false;
        }
        if (fmt.format == VK_FORMAT_B8G8R8A8_UNORM || fmt.format == VK_FORMAT_B8G8R8A8_SRGB || fmt.format ==
    ...
    auto colorSpaceToVkSurfaceFormat = [](vk::ColorSpace colorSpace, bool isBGR,
                                          bool hasSwapchainColorspaceExt) -> VkSurfaceFormatKHR
    {
      ...
      case vk::ColorSpace::SRGB_LINEAR:
        ...
        return VkSurfaceFormatKHR{
          isBGR ? VK_FORMAT_B8G8R8A8_UNORM : VK_FORMAT_R8G8B8A8_UNORM, VK_COLOR_SPACE_BT709_LINEAR_EXT
        };
      ...
      case vk::ColorSpace::HDR10:
        if (hasSwapchainColorspaceExt)
          return VkSurfaceFormatKHR{
            isBGR ? VK_FORMAT_A2B10G10R10_UNORM_PACK32 : VK_FORMAT_A2R10G10B10_UNORM_PACK32,
      ...
      default:
        ...
        return VkSurfaceFormatKHR{
          isBGR ? VK_FORMAT_B8G8R8A8_SRGB : VK_FORMAT_R8G8B8A8_SRGB, VK_COLOR_SPACE_SRGB_NONLINEAR_KHR
        };
      }
    };
    const VkSurfaceFormatKHR preferred = colorSpaceToVkSurfaceFormat(requestedColorSpace, isNativeSwapChainBGR(formats),
...
namespace vk
{
  struct DeferredTask
  {
    ...
    explicit DeferredTask(std::packaged_task<void()>&& task, SubmitHandle handle) : task_(std::move(task)),
                                                                                    handle_(handle)
    {
    }
  };
...
VkImageView vk::VulkanImage::createImageView(VkDevice device, VkImageViewType type, VkFormat format,
                                             VkImageAspectFlags aspectMask, uint32_t baseLevel, uint32_t numLevels,
                                             uint32_t baseLayer, uint32_t numLayers, const VkComponentMapping mapping,
                                             const VkSamplerYcbcrConversionInfo* ycbcr, const char* debugName) const
{
  PROFILER_FUNCTION_COLOR(PROFILER_COLOR_CREATE);
  const VkImageViewCreateInfo ci = {
    .sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO, .pNext = ycbcr, .image = vkImage_, .viewType = type,
    .format = format, .components = mapping,
...
void vk::VulkanImage::transitionLayout(VkCommandBuffer commandBuffer, VkImageLayout newImageLayout,
                                       const VkImageSubresourceRange& subresourceRange) const
{
  PROFILER_FUNCTION_COLOR(PROFILER_COLOR_BARRIER);
  const VkImageLayout oldImageLayout = vkImageLayout_ == VK_IMAGE_LAYOUT_ATTACHMENT_OPTIMAL
                                         ? (isDepthAttachment()
  ...
  const VkImageMemoryBarrier2 barrier = {
    .sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER_2, .srcStageMask = src.stage, .srcAccessMask = src.access,
    .dstStageMask = dst.stage, .dstAccessMask = dst.access, .oldLayout = vkImageLayout_, .newLayout = newImageLayout,
    .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .image = vkImage_,
...
void vk::VulkanImage::generateMipmap(VkCommandBuffer commandBuffer) const
{
  ...
  // Check if device supports downscaling for color or depth/stencil buffer based on image format
  {
    ...
    const bool hardwareDownscalingSupported = (vkFormatProperties_.optimalTilingFeatures & formatFeatureMask) ==
      formatFeatureMask;
  ...
  if (vkCmdBeginDebugUtilsLabelEXT)
  {
    const VkDebugUtilsLabelEXT utilsLabel = {
      .sType = VK_STRUCTURE_TYPE_DEBUG_UTILS_LABEL_EXT, .pLabelName = "Generate mipmaps",
  ...
  for (uint32_t layer = 0; layer < numLayers_; ++layer)
  {
    ...
    for (uint32_t i = 1; i < numLevels_; ++i)
    {
      ...
      imageMemoryBarrier2(commandBuffer, vkImage_,
      ...
      imageMemoryBarrier2(commandBuffer, vkImage_,
                          StageAccess{
                            ...
                            .stage = VK_PIPELINE_STAGE_2_TRANSFER_BIT, .access = VK_ACCESS_2_TRANSFER_READ_BIT
                          }, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                          /* oldImageLayout */
                          VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                          /* newImageLayout */
...
{
  return (format == VK_FORMAT_D16_UNORM) || (format == VK_FORMAT_X8_D24_UNORM_PACK32) || (format ==
    VK_FORMAT_D32_SFLOAT) || (format == VK_FORMAT_D16_UNORM_S8_UINT) || (format == VK_FORMAT_D24_UNORM_S8_UINT) || (
    format == VK_FORMAT_D32_SFLOAT_S8_UINT);
}

bool vk::VulkanImage::isStencilFormat(VkFormat format)
{
  return (format == VK_FORMAT_S8_UINT) || (format == VK_FORMAT_D16_UNORM_S8_UINT) || (format ==
    VK_FORMAT_D24_UNORM_S8_UINT) || (format == VK_FORMAT_D32_SFLOAT_S8_UINT);
}
...
VkImageView vk::VulkanImage::getOrCreateVkImageViewForFramebuffer(VulkanContext& ctx, uint8_t level, uint16_t layer)
{
  ...
  snprintf(debugNameImageView, sizeof(debugNameImageView) - 1, "Image View: '%s' imageViewForFramebuffer_[%u][%u]",
           debugName_, level, layer);
  imageViewForFramebuffer_[level][layer] = createImageView(ctx.getVkDevice(), VK_IMAGE_VIEW_TYPE_2D, vkImageFormat_,
                                                           getImageAspectFlags(), level, 1u, layer, 1u, {}, nullptr,
                                                           debugNameImageView);
  return imageViewForFramebuffer_[level][layer];
}
...
vk::Result vk::VulkanSwapchain::init(const InitInfo& info)
{
  ...
  VkResult result =
    vkGetPhysicalDeviceSurfaceSupportKHR(physicalDevice_, queueFamilyIndex_, surface_, &supportsPresent);
...
vk::Result vk::VulkanSwapchain::initResources(uint32_t& outWidth, uint32_t& outHeight, bool vSync)
{
  ...
  const VkSwapchainCreateInfoKHR createInfo = {
    .sType = VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR, .surface = surface_, .minImageCount = imageCount,
    .imageFormat = surfaceFormat_.format, .imageColorSpace = surfaceFormat_.colorSpace, .imageExtent = extent_,
    .imageArrayLayers = 1, .imageUsage = selectUsageFlags(), .imageSharingMode = VK_SHARING_MODE_EXCLUSIVE,
    .queueFamilyIndexCount = 1, .pQueueFamilyIndices = &queueFamilyIndex_,
    .preTransform = (surfaceCapabilities_.supportedTransforms & VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR)
                      ? VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR
                      : surfaceCapabilities_.currentTransform,
    .compositeAlpha = selectCompositeAlpha(surfaceCapabilities_), .presentMode = presentMode_, .clipped = VK_TRUE,
  ...
  for (uint32_t i = 0; i < numSwapchainImages_; i++)
  {
    ...
    VulkanImage vulkanImg = {
      .vkImage_ = swapImg.image, .vkUsageFlags_ = selectUsageFlags(), .vkExtent_ = {extent_.width, extent_.height, 1},
      .vkType_ = VK_IMAGE_TYPE_2D, .vkImageFormat_ = surfaceFormat_.format, .vkSamples_ = VK_SAMPLE_COUNT_1_BIT,
      .isSwapchainImage_ = true, .isOwningVkImage_ = false, .numLevels_ = 1, .numLayers_ = 1,
      ...
      .vkImageLayout_ = VK_IMAGE_LAYOUT_UNDEFINED, .imageView_ = swapImg.imageView,
    };
...
vk::Result vk::VulkanSwapchain::reinitResources(uint32_t& outWidth, uint32_t& outHeight, bool vSync)
{
  ...
  const VkSwapchainCreateInfoKHR createInfo = {
    ...
    .imageArrayLayers = 1, .imageUsage = selectUsageFlags(), .imageSharingMode = VK_SHARING_MODE_EXCLUSIVE,
    .queueFamilyIndexCount = 1, .pQueueFamilyIndices = &queueFamilyIndex_,
    .preTransform = surfaceCapabilities_.currentTransform, .compositeAlpha = selectCompositeAlpha(surfaceCapabilities_),
    .presentMode = presentMode_, .clipped = VK_TRUE, .oldSwapchain = oldSwapchain,
  };
...
vk::Result vk::VulkanSwapchain::acquireNextImage()
{
  ...
  if (frame.timelineWaitValue > 0)
  {
    const VkSemaphoreWaitInfo waitInfo = {
      .sType = VK_STRUCTURE_TYPE_SEMAPHORE_WAIT_INFO, .semaphoreCount = 1, .pSemaphores = &ctx_->timelineSemaphore_,
      .pValues = &frame.timelineWaitValue,
    };
  ...
  VkResult result = vkAcquireNextImageKHR(device_, swapchain_, UINT64_MAX, frame.imageAvailableSemaphore,
                                          // This will be signaled when image is available
                                          VK_NULL_HANDLE, &acquiredImageIndex_ // Use separate tracking variable
  );
  switch (result)
  {
  case VK_SUCCESS:
...
vk::Result vk::VulkanSwapchain::present(VkSemaphore waitSemaphore)
{
  ...
  const VkSwapchainPresentFenceInfoEXT fenceInfo = {
    .sType = VK_STRUCTURE_TYPE_SWAPCHAIN_PRESENT_FENCE_INFO_EXT, .swapchainCount = 1, .pFences = &frame.presentFence,
  };
  const VkPresentInfoKHR presentInfo = {
    .sType = VK_STRUCTURE_TYPE_PRESENT_INFO_KHR,
    .pNext = (ctx_->has_EXT_swapchain_maintenance1_ && frame.presentFence) ? &fenceInfo : nullptr,
    .waitSemaphoreCount = 1, .pWaitSemaphores = &waitSemaphore, .swapchainCount = 1, .pSwapchains = &swapchain_,
...
VkSurfaceFormatKHR vk::VulkanSwapchain::selectSwapSurfaceFormat(
...
VkPresentModeKHR vk::VulkanSwapchain::selectSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes,
                                                            bool vSync) const
{
  if (vSync)
  {
...
VkExtent2D vk::VulkanSwapchain::selectSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities, uint32_t requestedWidth,
...
VkCompositeAlphaFlagBitsKHR vk::VulkanSwapchain::selectCompositeAlpha(
...
void vk::VulkanSwapchain::createSynchronizationObjects()
{
  ...
  for (uint32_t i = 0; i < numSwapchainImages_; i++)
  {
    ...
    VK_ASSERT(
      vk::setDebugObjectName(device_, VK_OBJECT_TYPE_SEMAPHORE, (uint64_t)frame.imageAvailableSemaphore, debugName));
...
void vk::VulkanSwapchain::transitionImagesToPresentLayout()
{
  ...
  for (uint32_t i = 0; i < numSwapchainImages_; i++)
  {
    const VkImageMemoryBarrier2 barrier = {
      ...
      .dstAccessMask = VK_ACCESS_2_NONE, .oldLayout = VK_IMAGE_LAYOUT_UNDEFINED,
      .newLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
      .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .image = images_[i].image,
      .subresourceRange = {
...
vk::VulkanImmediateCommands::VulkanImmediateCommands(VkDevice device, uint32_t queueFamilyIndex,
                                                     const char* debugName) : device_(device),
                                                                              ...
                                                                              debugName_(debugName)
{
  ...
  const VkCommandBufferAllocateInfo ai = {
    .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO, .commandPool = commandPool_,
...
vk::SubmitHandle vk::VulkanImmediateCommands::submit(const CommandBufferWrapper& wrapper)
{
  ...
  VkSemaphoreSubmitInfo signalSemaphores[] = {
    VkSemaphoreSubmitInfo{
      .sType = VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO, .semaphore = wrapper.semaphore_,
  ...
  PROFILER_ZONE("vkQueueSubmit2()", PROFILER_COLOR_SUBMIT);
    ...
    const VkCommandBufferSubmitInfo bufferSI = {
      .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_SUBMIT_INFO, .commandBuffer = wrapper.cmdBuf_,
    };
    const VkSubmitInfo2 si = {
      .sType = VK_STRUCTURE_TYPE_SUBMIT_INFO_2, .waitSemaphoreInfoCount = numWaitSemaphores,
      .pWaitSemaphoreInfos = waitSemaphores, .commandBufferInfoCount = 1u, .pCommandBufferInfos = &bufferSI,
      .signalSemaphoreInfoCount = numSignalSemaphores, .pSignalSemaphoreInfos = signalSemaphores,
    };
...
vk::VulkanPipelineBuilder::VulkanPipelineBuilder() :
  vertexInputState_(VkPipelineVertexInputStateCreateInfo{
    .sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO, .vertexBindingDescriptionCount = 0,
    .pVertexBindingDescriptions = nullptr, .vertexAttributeDescriptionCount = 0,
    .pVertexAttributeDescriptions = nullptr,
  }),
  inputAssembly_({
    .sType = VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO, .flags = 0,
    .topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, .primitiveRestartEnable = VK_FALSE,
  }),
  rasterizationState_({
    .sType = VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_STATE_CREATE_INFO, .flags = 0, .depthClampEnable = VK_FALSE,
    .rasterizerDiscardEnable = VK_FALSE, .polygonMode = VK_POLYGON_MODE_FILL, .cullMode = VK_CULL_MODE_NONE,
    .frontFace = VK_FRONT_FACE_COUNTER_CLOCKWISE, .depthBiasEnable = VK_FALSE, .depthBiasConstantFactor = 0.0f,
    .depthBiasClamp = 0.0f, .depthBiasSlopeFactor = 0.0f, .lineWidth = 1.0f,
  }),
  multisampleState_({
    .sType = VK_STRUCTURE_TYPE_PIPELINE_MULTISAMPLE_STATE_CREATE_INFO, .rasterizationSamples = VK_SAMPLE_COUNT_1_BIT,
    .sampleShadingEnable = VK_FALSE, .minSampleShading = 0.0f, .pSampleMask = nullptr,
    .alphaToCoverageEnable = VK_FALSE, .alphaToOneEnable = VK_FALSE,
  }),
  depthStencilState_({
    .sType = VK_STRUCTURE_TYPE_PIPELINE_DEPTH_STENCIL_STATE_CREATE_INFO, .pNext = NULL, .flags = 0,
    .depthTestEnable = VK_FALSE, .depthWriteEnable = VK_FALSE, .depthCompareOp = VK_COMPARE_OP_LESS,
    .depthBoundsTestEnable = VK_FALSE, .stencilTestEnable = VK_FALSE,
    .front = {
      .failOp = VK_STENCIL_OP_KEEP, .passOp = VK_STENCIL_OP_KEEP, .depthFailOp = VK_STENCIL_OP_KEEP,
  ...
  tessellationState_({
    .sType = VK_STRUCTURE_TYPE_PIPELINE_TESSELLATION_STATE_CREATE_INFO, .flags = 0, .patchControlPoints = 0,
  })
{
}
...
vk::VulkanPipelineBuilder& vk::VulkanPipelineBuilder::rasterizationSamples(
  VkSampleCountFlagBits samples, float minSampleShading)
{
...
vk::VulkanPipelineBuilder& vk::VulkanPipelineBuilder::vertexInputState(
...
vk::VulkanPipelineBuilder& vk::VulkanPipelineBuilder::colorAttachments(
  const VkPipelineColorBlendAttachmentState* states, const VkFormat* formats, uint32_t numColorAttachments)
{
  ASSERT(states);
  ASSERT(formats);
...
vk::VulkanPipelineBuilder& vk::VulkanPipelineBuilder::stencilStateOps(VkStencilFaceFlags faceMask, VkStencilOp failOp,
...
vk::VulkanPipelineBuilder& vk::VulkanPipelineBuilder::stencilMasks(VkStencilFaceFlags faceMask, uint32_t compareMask,
...
VkResult vk::VulkanPipelineBuilder::build(VkDevice device, VkPipelineCache pipelineCache,
                                          VkPipelineLayout pipelineLayout, VkPipeline* outPipeline,
                                          const char* debugName) noexcept
{
  const VkPipelineDynamicStateCreateInfo dynamicState = {
    .sType = VK_STRUCTURE_TYPE_PIPELINE_DYNAMIC_STATE_CREATE_INFO, .dynamicStateCount = numDynamicStates_,
  ...
  const VkPipelineViewportStateCreateInfo viewportState = {
    .sType = VK_STRUCTURE_TYPE_PIPELINE_VIEWPORT_STATE_CREATE_INFO, .viewportCount = 1, .pViewports = nullptr,
    .scissorCount = 1, .pScissors = nullptr,
  };
  const VkPipelineColorBlendStateCreateInfo colorBlendState = {
    .sType = VK_STRUCTURE_TYPE_PIPELINE_COLOR_BLEND_STATE_CREATE_INFO, .logicOpEnable = VK_FALSE,
    .logicOp = VK_LOGIC_OP_COPY, .attachmentCount = numColorAttachments_, .pAttachments = colorBlendAttachmentStates_,
  };
  const VkPipelineRenderingCreateInfo renderingInfo = {
    .sType = VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR, .pNext = nullptr,
    .colorAttachmentCount = numColorAttachments_, .pColorAttachmentFormats = colorAttachmentFormats_,
    .depthAttachmentFormat = depthAttachmentFormat_, .stencilAttachmentFormat = stencilAttachmentFormat_,
  };
  const VkGraphicsPipelineCreateInfo ci = {
    .sType = VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO, .pNext = &renderingInfo, .flags = 0,
    .stageCount = numShaderStages_, .pStages = shaderStages_, .pVertexInputState = &vertexInputState_,
    .pInputAssemblyState = &inputAssembly_, .pTessellationState = &tessellationState_, .pViewportState = &viewportState,
    .pRasterizationState = &rasterizationState_, .pMultisampleState = &multisampleState_,
    .pDepthStencilState = &depthStencilState_, .pColorBlendState = &colorBlendState, .pDynamicState = &dynamicState,
    .layout = pipelineLayout, .renderPass = VK_NULL_HANDLE, .subpass = 0, .basePipelineHandle = VK_NULL_HANDLE,
    .basePipelineIndex = -1,
  };
...
void vk::CommandBuffer::cmdDispatchThreadGroups(const Dimensions& threadgroupCount, const Dependencies& deps)
{
  ...
  for (uint32_t i = 0; i != Dependencies::MAX_SUBMIT_DEPENDENCIES && deps.buffers[i]; i++)
  {
    ...
    ASSERT_MSG(buf->vkUsageFlags_ & VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
               "Did you forget to specify BufferUsageBits_Storage on your buffer?");
    bufferBarrier(deps.buffers[i], VK_PIPELINE_STAGE_2_VERTEX_SHADER_BIT | VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT,
...
void vk::CommandBuffer::cmdPushDebugGroupLabel(const char* label, uint32_t colorRGBA) const
{
  ...
  const VkDebugUtilsLabelEXT utilsLabel = {
    .sType = VK_STRUCTURE_TYPE_DEBUG_UTILS_LABEL_EXT, .pNext = nullptr, .pLabelName = label,
    .color = {
      float((colorRGBA >> 0) & 0xff) / 255.0f, float((colorRGBA >> 8) & 0xff) / 255.0f,
...
void vk::CommandBuffer::useComputeTexture(TextureHandle handle, VkPipelineStageFlags2 dstStage)
{
  ...
  {
    ASSERT_MSG(
      false, "Did you forget to specify TextureUsageBits::Storage or TextureUsageBits::Sampled on your texture?");
    return;
  }
  tex.transitionLayout(wrapper_->cmdBuf_,
                       ...
                       VkImageSubresourceRange{
                         tex.getImageAspectFlags(), 0, VK_REMAINING_MIP_LEVELS, 0, VK_REMAINING_ARRAY_LAYERS
                       });
}

void vk::CommandBuffer::bufferBarrier(BufferHandle handle, VkPipelineStageFlags2 srcStage,
                                      VkPipelineStageFlags2 dstStage)
{
  PROFILER_FUNCTION_COLOR(PROFILER_COLOR_BARRIER);
  VulkanBuffer* buf = ctx_->buffersPool_.get(handle);
  VkBufferMemoryBarrier2 barrier = {
    .sType = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER_2, .srcStageMask = srcStage, .srcAccessMask = 0,
    .dstStageMask = dstStage, .dstAccessMask = 0, .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
    .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .buffer = buf->vkBuffer_, .offset = 0, .size = VK_WHOLE_SIZE,
  };
...
void vk::CommandBuffer::cmdBeginRendering(const RenderPass& renderPass, const Framebuffer& fb, const Dependencies& deps)
{
  ...
  for (uint32_t i = 0; i != Dependencies::MAX_SUBMIT_DEPENDENCIES && deps.buffers[i]; i++)
  {
    VkPipelineStageFlags2 dstStageFlags = VK_PIPELINE_STAGE_2_VERTEX_SHADER_BIT |
      VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT;
    const VulkanBuffer* buf = ctx_->buffersPool_.get(deps.buffers[i]);
    ASSERT(buf);
    if ((buf->vkUsageFlags_ & VK_BUFFER_USAGE_INDEX_BUFFER_BIT) || (buf->vkUsageFlags_ &
  ...
  if (depthTex)
  {
    ...
    depthImg.transitionLayout(wrapper_->cmdBuf_, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
                              VkImageSubresourceRange{flags, 0, VK_REMAINING_MIP_LEVELS, 0, VK_REMAINING_ARRAY_LAYERS});
  }
  // handle depth MSAA
  if (TextureHandle handle = fb.depthStencil.resolveTexture)
  {
    ...
    depthResolveImg.transitionLayout(wrapper_->cmdBuf_, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
                                     VkImageSubresourceRange{
                                       flags, 0, VK_REMAINING_MIP_LEVELS, 0, VK_REMAINING_ARRAY_LAYERS
                                     });
  }
  ...
  for (uint32_t i = 0; i != numFbColorAttachments; i++)
  {
    ...
    colorAttachments[i] = {
      ...
      .resolveImageView = VK_NULL_HANDLE, .resolveImageLayout = VK_IMAGE_LAYOUT_UNDEFINED,
      .loadOp = loadOpToVkAttachmentLoadOp(descColor.loadOp),
      .storeOp = storeOpToVkAttachmentStoreOp(descColor.storeOp),
      .clearValue = {
        .color = {
          .float32 = {
            descColor.clearColor[0], descColor.clearColor[1], descColor.clearColor[2], descColor.clearColor[3]
          }
        }
      },
    };
  ...
  if (fb.depthStencil.texture)
  {
    ...
    depthAttachment = {
      ...
      .imageLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL, .resolveMode = VK_RESOLVE_MODE_NONE,
      .resolveImageView = VK_NULL_HANDLE, .resolveImageLayout = VK_IMAGE_LAYOUT_UNDEFINED,
  ...
  const VkRenderingInfo renderingInfo = {
    ...
    .layerCount = 1, .viewMask = 0, .colorAttachmentCount = numFbColorAttachments,
    .pColorAttachments = colorAttachments, .pDepthAttachment = depthTex ? &depthAttachment : nullptr,
...
void vk::CommandBuffer::cmdBindDepthState(const DepthState& desc)
{
  ...
  vkCmdSetDepthTestEnable(wrapper_->cmdBuf_,
                          (op != VK_COMPARE_OP_ALWAYS || desc.isDepthWriteEnabled) ? VK_TRUE : VK_FALSE);
...
void vk::CommandBuffer::cmdBindIndexBuffer(BufferHandle indexBuffer, IndexFormat indexFormat,
...
void vk::CommandBuffer::cmdPushConstants(const void* data, size_t size, size_t offset)
{
  ...
  VkShaderStageFlags shaderStageFlags = stateGraphics
                                          ...
                                          : (stateCompute
                                               ...
                                               : stateRayTracing->shaderStageFlags_);
...
void vk::CommandBuffer::cmdDraw(uint32_t vertexCount, uint32_t instanceCount, uint32_t firstVertex,
...
void vk::CommandBuffer::cmdDrawIndexed(uint32_t indexCount, uint32_t instanceCount, uint32_t firstIndex,
...
void vk::CommandBuffer::cmdDrawIndirect(BufferHandle indirectBuffer, size_t indirectBufferOffset, uint32_t drawCount,
                                        uint32_t stride)
{
  PROFILER_FUNCTION();
...
void vk::CommandBuffer::cmdDrawIndexedIndirect(BufferHandle indirectBuffer, size_t indirectBufferOffset,
...
void vk::CommandBuffer::cmdDrawIndexedIndirectCount(BufferHandle indirectBuffer, size_t indirectBufferOffset,
                                                    ...
                                                    uint32_t maxDrawCount, uint32_t stride)
{
  ...
  ASSERT(bufIndirect);
  ASSERT(bufCount);
  vkCmdDrawIndexedIndirectCount(wrapper_->cmdBuf_, bufIndirect->vkBuffer_, indirectBufferOffset, bufCount->vkBuffer_,
...
void vk::CommandBuffer::cmdDrawMeshTasksIndirect(BufferHandle indirectBuffer, size_t indirectBufferOffset,
                                                 uint32_t drawCount, uint32_t stride)
{
  PROFILER_FUNCTION();
...
void vk::CommandBuffer::cmdDrawMeshTasksIndirectCount(BufferHandle indirectBuffer, size_t indirectBufferOffset,
                                                      BufferHandle countBuffer, size_t countBufferOffset,
                                                      uint32_t maxDrawCount, uint32_t stride)
{
  PROFILER_FUNCTION();
...
void vk::CommandBuffer::cmdTraceRays(uint32_t width, uint32_t height, uint32_t depth, const Dependencies& deps)
{
  ...
  for (uint32_t i = 0; i != Dependencies::MAX_SUBMIT_DEPENDENCIES && deps.buffers[i]; i++)
  {
    bufferBarrier(deps.buffers[i], VK_PIPELINE_STAGE_2_VERTEX_INPUT_BIT | VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT,
...
void vk::CommandBuffer::cmdClearColorImage(TextureHandle tex, const ClearColorValue& value, const TextureLayers& layers)
{
  ...
  const VkImageSubresourceRange range = {
    .aspectMask = img->getImageAspectFlags(), .baseMipLevel = layers.mipLevel, .levelCount = VK_REMAINING_MIP_LEVELS,
    .baseArrayLayer = layers.layer, .layerCount = layers.numLayers,
  };
  imageMemoryBarrier2(wrapper_->cmdBuf_, img->vkImage_,
                      StageAccess{
                        .stage = VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT,
                        .access = VK_ACCESS_2_MEMORY_READ_BIT | VK_ACCESS_2_MEMORY_WRITE_BIT
                      }, StageAccess{
...
void vk::CommandBuffer::cmdCopyBuffer(BufferHandle src, BufferHandle dst, size_t size, size_t srcOffset,
                                      size_t dstOffset)
{
  ...
  const VkBufferCopy region = {.srcOffset = srcOffset, .dstOffset = dstOffset, .size = size,};
...
void vk::CommandBuffer::cmdCopyImage(TextureHandle src, TextureHandle dst, const Dimensions& extent,
                                     ...
                                     const TextureLayers& srcLayers, const TextureLayers& dstLayers)
{
  ...
  const VkImageSubresourceRange rangeSrc = {
    .aspectMask = imgSrc->getImageAspectFlags(), .baseMipLevel = srcLayers.mipLevel, .levelCount = 1,
    .baseArrayLayer = srcLayers.layer, .layerCount = srcLayers.numLayers,
  };
  const VkImageSubresourceRange rangeDst = {
    .aspectMask = imgDst->getImageAspectFlags(), .baseMipLevel = dstLayers.mipLevel, .levelCount = 1,
    .baseArrayLayer = dstLayers.layer, .layerCount = dstLayers.numLayers,
  };
  ASSERT(imgSrc->vkImageLayout_ != VK_IMAGE_LAYOUT_UNDEFINED);
  const VkExtent3D dstExtent = imgDst->vkExtent_;
  const bool coversFullDstImage = dstExtent.width == extent.width && dstExtent.height == extent.height && dstExtent.
    depth == extent.depth && dstOffset.x == 0 && dstOffset.y == 0 && dstOffset.z == 0;
  ...
  imageMemoryBarrier2(wrapper_->cmdBuf_, imgDst->vkImage_,
                      StageAccess{
                        .stage = VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT,
                        .access = VK_ACCESS_2_MEMORY_READ_BIT | VK_ACCESS_2_MEMORY_WRITE_BIT
                      }, StageAccess{
  ...
  const VkImageCopy regionCopy = {
    .srcSubresource = {
      .aspectMask = imgSrc->getImageAspectFlags(), .mipLevel = srcLayers.mipLevel, .baseArrayLayer = srcLayers.layer,
      .layerCount = srcLayers.numLayers,
    },
    .srcOffset = {.x = srcOffset.x, .y = srcOffset.y, .z = srcOffset.z},
    .dstSubresource = {
      .aspectMask = imgDst->getImageAspectFlags(), .mipLevel = dstLayers.mipLevel, .baseArrayLayer = dstLayers.layer,
  ...
  const VkImageBlit regionBlit = {
    ...
    .srcOffsets = {
      {},
      {
        .x = int32_t(srcOffset.x + imgSrc->vkExtent_.width), .y = int32_t(srcOffset.y + imgSrc->vkExtent_.height),
    ...
    .dstOffsets = {
      {},
      {
        .x = int32_t(dstOffset.x + imgDst->vkExtent_.width), .y = int32_t(dstOffset.y + imgDst->vkExtent_.height),
  ...
  isCompatible
    ? vkCmdCopyImage(wrapper_->cmdBuf_, imgSrc->vkImage_, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, imgDst->vkImage_,
                     VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &regionCopy)
    : vkCmdBlitImage(wrapper_->cmdBuf_, imgSrc->vkImage_, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, imgDst->vkImage_,
                     VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &regionBlit, VK_FILTER_LINEAR);
  imageMemoryBarrier2(wrapper_->cmdBuf_, imgSrc->vkImage_,
                      StageAccess{.stage = VK_PIPELINE_STAGE_2_TRANSFER_BIT, .access = VK_ACCESS_2_TRANSFER_READ_BIT},
                      StageAccess{
                        .stage = VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT,
  ...
  imageMemoryBarrier2(wrapper_->cmdBuf_, imgDst->vkImage_,
                      ...
                      StageAccess{
                        .stage = VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT,
                        .access = VK_ACCESS_2_MEMORY_READ_BIT | VK_ACCESS_2_MEMORY_WRITE_BIT
                      }, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, newLayout, rangeDst);
...
void vk::CommandBuffer::cmdUpdateTLAS(AccelStructHandle handle, BufferHandle instancesBuffer)
{
  ...
  VkAccelerationStructureBuildGeometryInfoKHR accelerationStructureBuildGeometryInfo{
    ...
    .geometryCount = 1, .pGeometries = &accelerationStructureGeometry,
  };
  ...
  vkGetAccelerationStructureBuildSizesKHR(ctx_->getVkDevice(), VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR,
                                          &accelerationStructureBuildGeometryInfo, &as->buildRangeInfo.primitiveCount,
  ...
  if (!as->scratchBuffer.valid() || bufferSize(*ctx_, as->scratchBuffer) < accelerationStructureBuildSizesInfo.
    updateScratchSize)
  {
    LOG_DEBUG("Recreating scratch buffer for TLAS update");
    as->scratchBuffer = ctx_->createBuffer(BufferDesc{
                                             .usage = BufferUsageBits_Storage, .storage = StorageType::Device,
  ...
  const VkAccelerationStructureBuildGeometryInfoKHR accelerationBuildGeometryInfo = {
    ...
    .flags = VK_BUILD_ACCELERATION_STRUCTURE_PREFER_FAST_TRACE_BIT_KHR |
    VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_UPDATE_BIT_KHR,
    .mode = VK_BUILD_ACCELERATION_STRUCTURE_MODE_UPDATE_KHR, .srcAccelerationStructure = as->vkHandle,
    .dstAccelerationStructure = as->vkHandle, .geometryCount = 1, .pGeometries = &accelerationStructureGeometry,
    ...
    const VkBufferMemoryBarrier2 barriers[] = {
      {
        ...
        .srcStageMask = VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR, .srcAccessMask = VK_ACCESS_MEMORY_READ_BIT,
        .dstStageMask = VK_PIPELINE_STAGE_ACCELERATION_STRUCTURE_BUILD_BIT_KHR,
        .dstAccessMask = VK_ACCESS_MEMORY_READ_BIT | VK_ACCESS_MEMORY_WRITE_BIT, .buffer = getVkBuffer(ctx_, handle),
        .size = VK_WHOLE_SIZE,
      },
      {
        .sType = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER_2,
        ...
        .dstStageMask = VK_PIPELINE_STAGE_ACCELERATION_STRUCTURE_BUILD_BIT_KHR,
        .dstAccessMask = VK_ACCESS_MEMORY_READ_BIT, .buffer = getVkBuffer(ctx_, instancesBuffer), .size = VK_WHOLE_SIZE,
      },
    };
    const VkDependencyInfo dependencyInfo{
      .sType = VK_STRUCTURE_TYPE_DEPENDENCY_INFO, .bufferMemoryBarrierCount = std::size(barriers),
                                      ...
                                      accelerationBuildStructureRangeInfos);
  {
    const VkBufferMemoryBarrier2 barrier = {
      ...
      .dstStageMask = VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR, .dstAccessMask = VK_ACCESS_MEMORY_READ_BIT,
...
void vk::VulkanStagingDevice::bufferSubData(VulkanBuffer& buffer, size_t dstOffset, size_t size, const void* data)
{
  ...
  while (size)
  {
    ...
    VkBufferMemoryBarrier barrier = {
      .sType = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER, .srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT,
      .dstAccessMask = 0, .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
      .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .buffer = buffer.vkBuffer_, .offset = dstOffset,
      .size = chunkSize,
    };
    ...
    vkCmdPipelineBarrier(wrapper.cmdBuf_, VK_PIPELINE_STAGE_TRANSFER_BIT, dstMask, VkDependencyFlags{}, 0, nullptr, 1,
                         &barrier, 0, nullptr);
...
void vk::VulkanStagingDevice::imageData2D(VulkanImage& image, const VkRect2D& imageRegion, uint32_t baseMipLevel,
                                          uint32_t numMipLevels, uint32_t layer, uint32_t numLayers, VkFormat format,
                                          const void* data)
{
  PROFILER_FUNCTION();
  ...
  ASSERT_MSG(
    !imageRegion.offset.x && !imageRegion.offset.y && imageRegion.extent.width == width && imageRegion.extent.height ==
    height, "Uploading mip-levels with an image region that is smaller than the base mip level is not supported");
    ...
    for (uint32_t i = 0; i != numLayers; i++)
    {
      ...
      for (uint32_t plane = 0; plane != numPlanes; plane++)
      {
        ...
        const VkRect2D region = {
          .offset = {.x = imageRegion.offset.x >> mipLevel, .y = imageRegion.offset.y >> mipLevel}, .extent = extent,
        };
        const VkBufferImageCopy copy = {
          // the offset for this level is at the start of all mip-levels plus the size of all previous mip-levels being uploaded
          .bufferOffset = desc.offset_ + offset + planeOffset, .bufferRowLength = 0, .bufferImageHeight = 0,
          .imageSubresource = VkImageSubresourceLayers{
            numPlanes > 1 ? VK_IMAGE_ASPECT_PLANE_0_BIT << plane : imageAspect, currentMipLevel, i, 1
          },
        ...
        planeOffset += getTextureBytesPerPlane(imageRegion.extent.width, imageRegion.extent.height,
                                               vkFormatToFormat(format), plane);
      }
      ...
      imageMemoryBarrier2(wrapper.cmdBuf_, image.vkImage_,
                          ...
                          VkImageSubresourceRange{imageAspect, currentMipLevel, 1, i, 1});
      offset += getTextureBytesPerLayer(imageRegion.extent.width, imageRegion.extent.height, texFormat,
                                        currentMipLevel);
    }
  }
...
void vk::VulkanStagingDevice::imageData3D(VulkanImage& image, const VkOffset3D& offset, const VkExtent3D& extent,
                                          VkFormat format, const void* data)
{
  ...
  // 2. Copy the pixel data from the staging buffer into the image
  const VkBufferImageCopy copy = {
    .bufferOffset = desc.offset_, .bufferRowLength = 0, .bufferImageHeight = 0,
    .imageSubresource = VkImageSubresourceLayers{VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1}, .imageOffset = offset,
    .imageExtent = extent,
  };
  vkCmdCopyBufferToImage(wrapper.cmdBuf_, stagingBuffer->vkBuffer_, image.vkImage_,
                         VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copy);
...
void vk::VulkanStagingDevice::getImageData(VulkanImage& image, const VkOffset3D& offset, const VkExtent3D& extent,
                                           VkImageSubresourceRange range, VkFormat format, void* outData)
{
  ...
  const VkBufferImageCopy copy = {
    .bufferOffset = desc.offset_, .bufferRowLength = 0, .bufferImageHeight = extent.height,
    .imageSubresource = VkImageSubresourceLayers{
      .aspectMask = range.aspectMask, .mipLevel = range.baseMipLevel, .baseArrayLayer = range.baseArrayLayer,
      .layerCount = range.layerCount,
    },
    .imageOffset = offset, .imageExtent = extent,
  };
...
void vk::VulkanStagingDevice::ensureStagingBufferSize(uint32_t sizeNeeded)
{
  ...
  stagingBuffer_ = {
    &ctx_,
    ctx_.createBuffer(stagingBufferSize_, VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
        ...
        SCOPE_EXIT
        {
          ...
          if (unusedSize > 0)
          {
            regions_.insert(regions_.begin(), {
                              .offset_ = unusedOffset, .size_ = unusedSize, .handle_ = SubmitHandle()
                            });
          }
        };
...
vk::SubmitHandle vk::VulkanContext::submit(ICommandBuffer& commandBuffer, TextureHandle present)
{
  ...
  if (present)
  {
    ...
    tex.transitionLayout(vkCmdBuffer->wrapper_->cmdBuf_, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, VkImageSubresourceRange{
                           VK_IMAGE_ASPECT_COLOR_BIT, 0, VK_REMAINING_MIP_LEVELS, 0, VK_REMAINING_ARRAY_LAYERS
                         });
  }
...
vk::Holder<vk::BufferHandle> vk::VulkanContext::createBuffer(const BufferDesc& requestedDesc, const char* debugName,
                                                             Result* outResult)
{
...
vk::Holder<vk::QueryPoolHandle> vk::VulkanContext::createQueryPool(uint32_t numQueries, const char* debugName,
                                                                   Result* outResult)
{
  PROFILER_FUNCTION();
  const VkQueryPoolCreateInfo createInfo = {
    .sType = VK_STRUCTURE_TYPE_QUERY_POOL_CREATE_INFO, .flags = 0, .queryType = VK_QUERY_TYPE_TIMESTAMP,
    .queryCount = numQueries, .pipelineStatistics = 0,
  };
...
vk::Holder<vk::AccelStructHandle> vk::VulkanContext::createAccelerationStructure(
  const AccelStructDesc& desc, Result* outResult)
{
  PROFILER_FUNCTION();
...
vk::Holder<vk::TextureHandle> vk::VulkanContext::createTexture(const TextureDesc& requestedDesc, const char* debugName,
                                                               Result* outResult)
{
  PROFILER_FUNCTION_COLOR(PROFILER_COLOR_CREATE);
  ...
  const VkFormat vkFormat = isDepthOrStencilFormat(desc.format)
  ...
  {
    usageFlags |= isDepthOrStencilFormat(desc.format)
  ...
  VulkanImage image = {
    .vkUsageFlags_ = usageFlags, .vkExtent_ = vkExtent, .vkType_ = vkImageType, .vkImageFormat_ = vkFormat,
    .vkSamples_ = vkSamples, .numLevels_ = numLevels, .numLayers_ = numLayers,
    .isDepthFormat_ = VulkanImage::isDepthFormat(vkFormat), .isStencilFormat_ = VulkanImage::isStencilFormat(vkFormat),
  };
  if (hasDebugName)
  {
    // store debug name
  ...
  const VkImageCreateInfo ci = {
    .sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO, .pNext = nullptr, .flags = vkCreateFlags, .imageType = vkImageType,
    .format = vkFormat, .extent = vkExtent, .mipLevels = numLevels, .arrayLayers = numLayers, .samples = vkSamples,
    .tiling = VK_IMAGE_TILING_OPTIMAL, .usage = usageFlags, .sharingMode = VK_SHARING_MODE_EXCLUSIVE,
    .queueFamilyIndexCount = 0, .pQueueFamilyIndices = nullptr, .initialLayout = VK_IMAGE_LAYOUT_UNDEFINED,
  };
  if (numPlanes == 1)
  {
    ...
    if (!VERIFY(result == VK_SUCCESS))
    {
      LOG_WARNING("Failed: error result: {}, memflags: {},  imageformat: {}\n", string_VkResult(result),
                  string_VkMemoryPropertyFlags(memFlags), string_VkFormat(image.vkImageFormat_));
  ...
  const VkComponentMapping mapping = {
    .r = VkComponentSwizzle(desc.swizzle.r), .g = VkComponentSwizzle(desc.swizzle.g),
    .b = VkComponentSwizzle(desc.swizzle.b), .a = VkComponentSwizzle(desc.swizzle.a),
  };
  const VkSamplerYcbcrConversionInfo* ycbcrInfo = isDisjoint ? getOrCreateYcbcrConversionInfo(desc.format) : nullptr;
  image.imageView_ = image.createImageView(vkDevice_, vkImageViewType, vkFormat, aspect, 0, VK_REMAINING_MIP_LEVELS, 0,
  ...
  if (image.vkUsageFlags_ & VK_IMAGE_USAGE_STORAGE_BIT)
  {
    if (!desc.swizzle.identity())
    {
      ...
      image.imageViewStorage_ = image.createImageView(vkDevice_, vkImageViewType, vkFormat, aspect, 0,
                                                      VK_REMAINING_MIP_LEVELS, 0, numLayers, {}, ycbcrInfo,
                                                      debugNameImageView);
  ...
  if (desc.data)
  {
    ...
    Result res = upload(handle, {
                          .dimensions = desc.dimensions, .numLayers = num_layers, .numMipLevels = desc.dataNumMipLevels
                        }, desc.data);
    if (!res.isOk())
    {
...
vk::Holder<vk::TextureHandle> vk::VulkanContext::createTextureView(TextureHandle texture, const TextureViewDesc& desc,
                                                                   const char* debugName, Result* outResult)
{
  ...
  image.imageView_ = image.createImageView(vkDevice_, vkImageViewType, image.vkImageFormat_, aspect, desc.mipLevel,
  ...
  if (image.vkUsageFlags_ & VK_IMAGE_USAGE_STORAGE_BIT)
  {
    ...
    {
      // use identity swizzle for storage images
      image.imageViewStorage_ = image.createImageView(vkDevice_, vkImageViewType, image.vkImageFormat_, aspect,
                                                      desc.mipLevel, desc.numMipLevels, desc.layer, desc.numLayers, {},
                                                      nullptr, debugName);
      ASSERT(image.imageViewStorage_ != VK_NULL_HANDLE);
    }
  }
...
vk::AccelStructHandle vk::VulkanContext::createBLAS(const AccelStructDesc& desc, Result* outResult)
{
  ...
  AccelerationStructure accelStruct = {
    .buildRangeInfo = {
      .primitiveCount = desc.buildRange.primitiveCount, .primitiveOffset = desc.buildRange.primitiveOffset,
      .firstVertex = desc.buildRange.firstVertex, .transformOffset = desc.buildRange.transformOffset,
    },
    .buffer = createBuffer({
                             .usage = BufferUsageBits_AccelStructStorage, .storage = StorageType::Device,
                             .size = accelerationStructureBuildSizesInfo.accelerationStructureSize,
                             .debugName = debugNameBuffer,
                           }, nullptr, outResult),
  };
  const VkAccelerationStructureCreateInfoKHR ciAccelerationStructure = {
    .sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_CREATE_INFO_KHR, .buffer = getVkBuffer(this, accelStruct.buffer),
    .size = accelerationStructureBuildSizesInfo.accelerationStructureSize,
    .type = VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR,
  };
  VK_ASSERT(vkCreateAccelerationStructureKHR(vkDevice_, &ciAccelerationStructure, nullptr, &accelStruct.vkHandle));
  Holder<BufferHandle> scratchBuffer = createBuffer({
                                                      .usage = BufferUsageBits_Storage, .storage = StorageType::Device,
  ...
  const VkAccelerationStructureBuildGeometryInfoKHR accelerationBuildGeometryInfo{
    ...
    .mode = VK_BUILD_ACCELERATION_STRUCTURE_MODE_BUILD_KHR, .dstAccelerationStructure = accelStruct.vkHandle,
    .geometryCount = 1, .pGeometries = &accelerationStructureGeometry,
  ...
  vkCmdBuildAccelerationStructuresKHR(getVkCommandBuffer(buffer), 1, &accelerationBuildGeometryInfo,
                                      accelerationBuildStructureRangeInfos);
  wait(submit(buffer, {}));
...
static_assert(
  1 << (sizeof(vk::Format) * 8) <= (sizeof(vk::VulkanContextImpl::ycbcrConversionData_) / sizeof((
    vk::VulkanContextImpl::ycbcrConversionData_)[0])),
  "There aren't enough elements in `ycbcrConversionData_` to be accessed by vk::Format");
...
const VkSamplerYcbcrConversionInfo* vk::VulkanContext::getOrCreateYcbcrConversionInfo(Format format)
{
  ...
  const VkSamplerYcbcrConversionCreateInfo ci = {
    .sType = VK_STRUCTURE_TYPE_SAMPLER_YCBCR_CONVERSION_CREATE_INFO, .format = vkFormat,
    .ycbcrModel = VK_SAMPLER_YCBCR_MODEL_CONVERSION_YCBCR_709, .ycbcrRange = VK_SAMPLER_YCBCR_RANGE_ITU_FULL,
    .components = {
    ...
    .chromaFilter = VK_FILTER_LINEAR, .forceExplicitReconstruction = VK_FALSE,
  };
  ...
  VkImageFormatProperties2 imageFormatProps = {
    .sType = VK_STRUCTURE_TYPE_IMAGE_FORMAT_PROPERTIES_2, .pNext = &samplerYcbcrConversionImageFormatProps,
  };
  const VkPhysicalDeviceImageFormatInfo2 imageFormatInfo = {
    .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_IMAGE_FORMAT_INFO_2, .format = vkFormat, .type = VK_IMAGE_TYPE_2D,
    .tiling = VK_IMAGE_TILING_OPTIMAL, .usage = VK_IMAGE_USAGE_SAMPLED_BIT, .flags = VK_IMAGE_CREATE_DISJOINT_BIT,
  };
  ...
  pimpl_->ycbcrConversionData_[(uint8_t)format].sampler = {
...
VkPipeline vk::VulkanContext::getVkPipeline(RenderPipelineHandle handle, uint32_t viewMask)
{
  ...
  for (uint32_t i = 0; i != numColorAttachments; i++)
  {
    ...
    if (!attachment.blendEnabled)
    {
      colorBlendAttachmentStates[i] = VkPipelineColorBlendAttachmentState{
        ...
        .dstColorBlendFactor = VK_BLEND_FACTOR_ZERO, .colorBlendOp = VK_BLEND_OP_ADD,
    ...
    {
      colorBlendAttachmentStates[i] = VkPipelineColorBlendAttachmentState{
        .blendEnable = VK_TRUE, .srcColorBlendFactor = blendFactorToVkBlendFactor(attachment.srcRGBBlendFactor),
  ...
  if (tescModule || teseModule || desc.patchControlPoints)
  {
    ...
    ASSERT(
      desc.patchControlPoints > 0 && desc.patchControlPoints <= vkPhysicalDeviceProperties2_.properties.limits.
...
#undef UPDATE_PUSH_CONSTANT_SIZE
    ...
    const VkPipelineLayoutCreateInfo ci = {
      .sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO, .setLayoutCount = (uint32_t)std::size(dsls),
      .pSetLayouts = dsls, .pushConstantRangeCount = pushConstantsSize ? 1u : 0u,
  ...
  VulkanPipelineBuilder()
    // from Vulkan 1.0
    .dynamicState(VK_DYNAMIC_STATE_VIEWPORT).dynamicState(VK_DYNAMIC_STATE_SCISSOR).
    dynamicState(VK_DYNAMIC_STATE_DEPTH_BIAS).dynamicState(VK_DYNAMIC_STATE_BLEND_CONSTANTS)
    // from Vulkan 1.3 or VK_EXT_extended_dynamic_state
    .dynamicState(VK_DYNAMIC_STATE_DEPTH_TEST_ENABLE).dynamicState(VK_DYNAMIC_STATE_DEPTH_WRITE_ENABLE).dynamicState(
      VK_DYNAMIC_STATE_DEPTH_COMPARE_OP)
    // from Vulkan 1.3 or VK_EXT_extended_dynamic_state2
    .dynamicState(VK_DYNAMIC_STATE_DEPTH_BIAS_ENABLE).primitiveTopology(topologyToVkPrimitiveTopology(desc.topology)).
    rasterizationSamples(getVulkanSampleCountFlags(desc.samplesCount, getFramebufferMSAABitMask()),
                         desc.minSampleShading).polygonMode(polygonModeToVkPolygonMode(desc.polygonMode)).
    stencilStateOps(VK_STENCIL_FACE_FRONT_BIT, stencilOpToVkStencilOp(desc.frontFaceStencil.stencilFailureOp),
    ...
    stencilStateOps(VK_STENCIL_FACE_BACK_BIT, stencilOpToVkStencilOp(desc.backFaceStencil.stencilFailureOp),
    ...
    shaderStage(taskModule
                  ? getPipelineShaderStageCreateInfo(VK_SHADER_STAGE_TASK_BIT_EXT, taskModule->sm, desc.entryPointTask,
                                                     &si)
                  : VkPipelineShaderStageCreateInfo{.module = VK_NULL_HANDLE}).
    shaderStage(meshModule
                  ? getPipelineShaderStageCreateInfo(VK_SHADER_STAGE_MESH_BIT_EXT, meshModule->sm, desc.entryPointMesh,
                                                     &si)
                  : getPipelineShaderStageCreateInfo(VK_SHADER_STAGE_VERTEX_BIT, vertModule->sm, desc.entryPointVert,
                                                     &si)).
    shaderStage(
      getPipelineShaderStageCreateInfo(VK_SHADER_STAGE_FRAGMENT_BIT, fragModule->sm, desc.entryPointFrag, &si)).
    shaderStage(tescModule
                  ? getPipelineShaderStageCreateInfo(VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT, tescModule->sm,
                                                     desc.entryPointTesc, &si)
                  : VkPipelineShaderStageCreateInfo{.module = VK_NULL_HANDLE}).
    shaderStage(teseModule
                  ? getPipelineShaderStageCreateInfo(VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT, teseModule->sm,
                                                     desc.entryPointTese, &si)
                  : VkPipelineShaderStageCreateInfo{.module = VK_NULL_HANDLE}).
    shaderStage(geomModule
                  ? getPipelineShaderStageCreateInfo(VK_SHADER_STAGE_GEOMETRY_BIT, geomModule->sm, desc.entryPointGeom,
                                                     &si)
                  : VkPipelineShaderStageCreateInfo{.module = VK_NULL_HANDLE}).
    cullMode(cullModeToVkCullMode(desc.cullMode)).frontFace(windingModeToVkFrontFace(desc.frontFaceWinding)).
    ...
    stencilAttachmentFormat(formatToVkFormat(desc.stencilFormat)).patchControlPoints(desc.patchControlPoints).build(
...
#undef UPDATE_PUSH_CONSTANT_SIZE
    ...
    const VkPipelineLayoutCreateInfo ciPipelineLayout = {
      .sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO, .setLayoutCount = std::size(dsls), .pSetLayouts = dsls,
    ...
    VK_ASSERT(
      vk::setDebugObjectName(vkDevice_, VK_OBJECT_TYPE_PIPELINE_LAYOUT, (uint64_t)rtps->pipelineLayout_,
        pipelineLayoutName));
  }
  VkSpecializationMapEntry entries[SpecializationConstantDesc::SPECIALIZATION_CONSTANTS_MAX] = {};
...
#undef ADD_STAGE
  ...
  for (int i = 0; i < MAX_RAY_TRACING_SHADER_GROUP_SIZE; ++i)
  {
    if (moduleRGen[i])
    {
      ...
      shaderGroups[numShaderGroups++] = VkRayTracingShaderGroupCreateInfoKHR{
        ...
        .type = VK_RAY_TRACING_SHADER_GROUP_TYPE_GENERAL_KHR, .generalShader = numShaders++,
  ...
  const VkRayTracingPipelineCreateInfoKHR ciRayTracingPipeline = {
    .sType = VK_STRUCTURE_TYPE_RAY_TRACING_PIPELINE_CREATE_INFO_KHR, .stageCount = numShaderStages,
    .pStages = ciShaderStages, .groupCount = numShaderGroups, .pGroups = shaderGroups,
    .maxPipelineRayRecursionDepth = rayTracingPipelineProperties_.maxRayRecursionDepth, .layout = rtps->pipelineLayout_,
  };
  VK_ASSERT(
    vkCreateRayTracingPipelinesKHR(vkDevice_, VK_NULL_HANDLE, VK_NULL_HANDLE, 1, &ciRayTracingPipeline, nullptr, &rtps
      ->pipeline_));
  ...
  VK_ASSERT(
    vkGetRayTracingShaderGroupHandlesKHR(vkDevice_, rtps->pipeline_, 0, numShaderGroups, sbtSize, shaderHandleStorage.
      data()));
  ...
  rtps->sbt = createBuffer({
                             .usage = BufferUsageBits_ShaderBindingTable, .storage = StorageType::Device,
                             .size = sbtBufferSize, .data = sbtStorage.data(), .debugName = "Buffer: SBT",
                           }, nullptr, nullptr);
  // generate SBT entries
  rtps->sbtEntryRayGen = {
    .deviceAddress = gpuAddress(rtps->sbt), .stride = handleSizeAligned, .size = handleSizeAligned,
  };
  rtps->sbtEntryMiss = {
    .deviceAddress = idxMiss ? gpuAddress(rtps->sbt, idxMiss * sbtEntrySizeAligned) : 0, .stride = handleSizeAligned,
    .size = handleSizeAligned,
  };
  rtps->sbtEntryHit = {
    .deviceAddress = idxHit ? gpuAddress(rtps->sbt, idxHit * sbtEntrySizeAligned) : 0, .stride = handleSizeAligned,
    .size = handleSizeAligned,
  };
  rtps->sbtEntryCallable = {
    .deviceAddress = idxCallable ? gpuAddress(rtps->sbt, idxCallable * sbtEntrySizeAligned) : 0,
    .stride = handleSizeAligned, .size = handleSizeAligned,
  };
  return rtps->pipeline_;
}
  ...
  if (cps->pipeline_ == VK_NULL_HANDLE)
  {
    ...
    // create pipeline layout
    {
      ...
      const VkPushConstantRange range = {
        .stageFlags = VK_SHADER_STAGE_COMPUTE_BIT, .offset = 0, .size = sm->pushConstantsSize
      };
      const VkPipelineLayoutCreateInfo ci = {
        .sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO, .setLayoutCount = (uint32_t)std::size(dsls),
        .pSetLayouts = dsls, .pushConstantRangeCount = sm->pushConstantsSize ? 1u : 0u, // FIX: Only set if size > 0
      ...
      VK_ASSERT(
        vk::setDebugObjectName(vkDevice_, VK_OBJECT_TYPE_PIPELINE_LAYOUT, (uint64_t)cps->pipelineLayout_,
          pipelineLayoutName));
    }
    const VkComputePipelineCreateInfo ci = {
      .sType = VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO, .flags = 0,
      .stage = getPipelineShaderStageCreateInfo(VK_SHADER_STAGE_COMPUTE_BIT, sm->sm, cps->desc_.entryPoint, &siComp),
      .layout = cps->pipelineLayout_, .basePipelineHandle = VK_NULL_HANDLE, .basePipelineIndex = -1,
    };
    VK_ASSERT(vkCreateComputePipelines(vkDevice_, pipelineCache_, 1, &ci, nullptr, &cps->pipeline_));
    VK_ASSERT(
      vk::setDebugObjectName(vkDevice_, VK_OBJECT_TYPE_PIPELINE, (uint64_t)cps->pipeline_, cps->desc_.debugName));
  }
  return cps->pipeline_;
}

vk::Holder<vk::ComputePipelineHandle> vk::VulkanContext::createComputePipeline(
  const ComputePipelineDesc& desc, Result* outResult)
{
...
vk::Holder<vk::RayTracingPipelineHandle> vk::VulkanContext::createRayTracingPipeline(
  const RayTracingPipelineDesc& desc, Result* outResult)
{
  PROFILER_FUNCTION();
...
vk::Holder<vk::RenderPipelineHandle> vk::VulkanContext::createRenderPipeline(
  const RenderPipelineDesc& desc, Result* outResult)
{
  ...
  for (uint32_t i = 0; i != rps.numAttributes_; i++)
  {
    ...
    rps.vkAttributes_[i] = {
      .location = attr.location, .binding = attr.binding, .format = vertexFormatToVkFormat(attr.format),
    ...
    if (!bufferAlreadyBound[attr.binding])
    {
      ...
      rps.vkBindings_[rps.numBindings_++] = {
        .binding = attr.binding, .stride = vstate.inputBindings[attr.binding].stride,
  ...
  {
    {
      ...
      VulkanImage* tex = texturesPool_.get(handle);
      if (!tex || !tex->isOwningVkImage_) return;
      destroy(handle);
      handle = {};
    }
  };
...
vk::Result vk::VulkanContext::download(TextureHandle handle, const TextureRangeDesc& range, void* outData)
{
  ...
  stagingDevice_->getImageData(*texture, VkOffset3D{range.offset.x, range.offset.y, range.offset.z},
                               VkExtent3D{range.dimensions.width, range.dimensions.height, range.dimensions.depth},
                               VkImageSubresourceRange{
                                 .aspectMask = texture->getImageAspectFlags(), .baseMipLevel = range.mipLevel,
                                 .levelCount = range.numMipLevels, .baseArrayLayer = range.layer,
...
vk::Result vk::VulkanContext::upload(TextureHandle handle, const TextureRangeDesc& range, const void* data)
{
  ...
  {
    stagingDevice_->imageData3D(*texture, VkOffset3D{range.offset.x, range.offset.y, range.offset.z},
                                VkExtent3D{range.dimensions.width, range.dimensions.height, range.dimensions.depth},
                                vkFormat, data);
  }
  else
  {
    ...
    stagingDevice_->imageData2D(*texture, imageRegion, range.mipLevel, range.numMipLevels, range.layer, range.numLayers,
                                vkFormat, data);
  }
  return Result();
}
...
vk::Holder<vk::ShaderModuleHandle> vk::VulkanContext::createShaderModule(
  const ShaderModuleDesc& desc, Result* outResult)
{
  Result result;
  ShaderModuleState sm = desc.dataSize
                           ? createShaderModuleFromSPIRV(desc.data, desc.dataSize, desc.debugName, &result) // binary
...
vk::ShaderModuleState vk::VulkanContext::createShaderModuleFromSPIRV(const void* spirv, size_t numBytes,
                                                                     const char* debugName, Result* outResult) const
{
  VkShaderModule vkShaderModule = VK_NULL_HANDLE;
  const VkShaderModuleCreateInfo ci = {
    .sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO, .codeSize = numBytes, .pCode = (const uint32_t*)spirv,
  };
  {
...
vk::ShaderModuleState vk::VulkanContext::createShaderModuleFromGLSL(ShaderStage stage, const char* source,
                                                                    const char* debugName, Result* outResult) const
{
  ...
  if (strstr(source, "#version ") == nullptr)
  {
    ...
    if (vkStage == VK_SHADER_STAGE_VERTEX_BIT || vkStage == VK_SHADER_STAGE_COMPUTE_BIT || vkStage ==
      VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT || vkStage == VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT)
    {
      sourcePatched += R"(
      #version 460
      #extension GL_EXT_buffer_reference : require
...
bool vk::VulkanContext::getQueryPoolResults(QueryPoolHandle pool, uint32_t firstQuery, uint32_t queryCount,
                                            size_t dataSize, void* outData, size_t stride) const
{
  VkQueryPool vkPool = *queriesPool_.get(pool);
  VK_ASSERT(
    vkGetQueryPoolResults( vkDevice_, vkPool, firstQuery, queryCount, dataSize, outData, stride,
...
void vk::VulkanContext::createInstance()
{
  ...
  const VkValidationFeatureDisableEXT validationFeaturesDisabled[] = {
    VK_VALIDATION_FEATURE_DISABLE_SHADERS_EXT, VK_VALIDATION_FEATURE_DISABLE_SHADER_VALIDATION_CACHE_EXT,
  };
#endif // __APPLE__
  const VkValidationFeaturesEXT features = {
    .sType = VK_STRUCTURE_TYPE_VALIDATION_FEATURES_EXT, .pNext = nullptr,
#if !defined(__ANDROID__)
...
#undef LAYER_SETTINGS_BOOL32
  const VkLayerSettingsCreateInfoEXT layerSettingsCreateInfo = {
    .sType = VK_STRUCTURE_TYPE_LAYER_SETTINGS_CREATE_INFO_EXT, .pNext = config_.enableValidation ? &features : nullptr,
    .settingCount = (uint32_t)std::size(settings), .pSettings = settings,
  };
#endif // defined(VK_EXT_layer_settings) && VK_EXT_layer_settings
  const VkApplicationInfo appInfo = {
    .sType = VK_STRUCTURE_TYPE_APPLICATION_INFO, .pNext = nullptr, .pApplicationName = "Vulkan",
    .applicationVersion = VK_MAKE_VERSION(1, 0, 0), .pEngineName = "Vulkan", .engineVersion = VK_MAKE_VERSION(1, 0, 0),
...
#endif // defined(VK_EXT_layer_settings) && VK_EXT_layer_settings
    .flags = flags, .pApplicationInfo = &appInfo,
  ...
  if (hasDebugUtils)
  {
    const VkDebugUtilsMessengerCreateInfoEXT cdi = {
  ...
  LOG_INFO("{} layer version: {}.{}.{}\n", kDefaultValidationLayers[0], VK_VERSION_MAJOR(khronosValidationVersion_),
           VK_VERSION_MINOR(khronosValidationVersion_), VK_VERSION_PATCH(khronosValidationVersion_));
...
void vk::VulkanContext::addNextPhysicalDeviceProperties(void* properties)
{
  ...
  std::launder(reinterpret_cast<VkBaseOutStructure*>(properties))->pNext = std::launder(
    reinterpret_cast<VkBaseOutStructure*>(vkPhysicalDeviceProperties2_.pNext));
  vkPhysicalDeviceProperties2_.pNext = properties;
}

void vk::VulkanContext::getBuildInfoBLAS(const AccelStructDesc& desc, VkAccelerationStructureGeometryKHR& outGeometry,
                                         VkAccelerationStructureBuildSizesInfoKHR& outSizesInfo) const
{
  ...
  ASSERT(
    buffersPool_.get(desc.indexBuffer)->vkUsageFlags_ &
    VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR);
  ASSERT(
    buffersPool_.get(desc.vertexBuffer)->vkUsageFlags_ &
    VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR);
  ASSERT(
    buffersPool_.get(desc.transformBuffer)->vkUsageFlags_ &
    VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR);
  ...
  outGeometry = VkAccelerationStructureGeometryKHR{
    ...
    .geometry = {
      .triangles = {
        ...
        .vertexStride = desc.vertexStride ? desc.vertexStride : getVertexFormatSize(desc.vertexFormat),
        .maxVertex = desc.numVertices - 1, .indexType = VK_INDEX_TYPE_UINT32,
  ...
  const VkAccelerationStructureBuildGeometryInfoKHR accelerationBuildGeometryInfo{
    .sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_BUILD_GEOMETRY_INFO_KHR,
    .type = VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR,
    .flags = VK_BUILD_ACCELERATION_STRUCTURE_PREFER_FAST_TRACE_BIT_KHR, .geometryCount = 1, .pGeometries = &outGeometry,
  };
  outSizesInfo.sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_BUILD_SIZES_INFO_KHR;
  vkGetAccelerationStructureBuildSizesKHR(vkDevice_, VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR,
                                          &accelerationBuildGeometryInfo, &desc.buildRange.primitiveCount,
                                          &outSizesInfo);
...
void vk::VulkanContext::getBuildInfoTLAS(const AccelStructDesc& desc, VkAccelerationStructureGeometryKHR& outGeometry,
                                         VkAccelerationStructureBuildSizesInfoKHR& outSizesInfo) const
{
  ...
  ASSERT(desc.buildRange.primitiveCount);
  ASSERT(
    buffersPool_.get(desc.instancesBuffer)->vkUsageFlags_ &
    VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR);
  outGeometry = VkAccelerationStructureGeometryKHR{
    .sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_GEOMETRY_KHR, .geometryType = VK_GEOMETRY_TYPE_INSTANCES_KHR,
    .geometry = {
  ...
  const VkAccelerationStructureBuildGeometryInfoKHR accelerationStructureBuildGeometryInfo = {
    .sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_BUILD_GEOMETRY_INFO_KHR,
    .type = VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR,
    .flags = buildFlagsToVkBuildAccelerationStructureFlags(desc.buildFlags), .geometryCount = 1,
    .pGeometries = &outGeometry,
  };
  outSizesInfo.sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_BUILD_SIZES_INFO_KHR,
    vkGetAccelerationStructureBuildSizesKHR(vkDevice_, VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR,
                                            &accelerationStructureBuildGeometryInfo, &desc.buildRange.primitiveCount,
                                            &outSizesInfo);
  const uint32_t alignment = accelerationStructureProperties_.minAccelerationStructureScratchOffsetAlignment;
  outSizesInfo.accelerationStructureSize += alignment;
  outSizesInfo.updateScratchSize += alignment;
...
vk::Result vk::VulkanContext::initContext(const HWDeviceDesc& desc)
{
  ...
  const VkDeviceQueueCreateInfo ciQueue[2] = {
    {
      .sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO, .queueFamilyIndex = deviceQueues_.graphicsQueueFamilyIndex,
      .queueCount = 1, .pQueuePriorities = &queuePriority,
    },
    {
      .sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO, .queueFamilyIndex = deviceQueues_.computeQueueFamilyIndex,
      .queueCount = 1, .pQueuePriorities = &queuePriority,
    },
  };
  ...
  VkPhysicalDeviceFeatures deviceFeatures10 = {
    .robustBufferAccess = vkFeatures10_.features.robustBufferAccess,
    .geometryShader = vkFeatures10_.features.geometryShader,
    // enable if supported
    .tessellationShader = vkFeatures10_.features.tessellationShader,
    // enable if supported
    .sampleRateShading = VK_TRUE, .multiDrawIndirect = VK_TRUE, .drawIndirectFirstInstance = VK_TRUE,
    .depthBiasClamp = VK_TRUE, .fillModeNonSolid = vkFeatures10_.features.fillModeNonSolid,
    // enable if supported
    .samplerAnisotropy = VK_TRUE, .textureCompressionBC = vkFeatures10_.features.textureCompressionBC,
    // enable if supported
    .vertexPipelineStoresAndAtomics = vkFeatures10_.features.vertexPipelineStoresAndAtomics,
    // enable if supported
    .fragmentStoresAndAtomics = VK_TRUE, .shaderImageGatherExtended = VK_TRUE,
  ...
  VkPhysicalDeviceVulkan11Features deviceFeatures11 = {
    .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_1_FEATURES, .pNext = config_.extensionsDeviceFeatures,
    .storageBuffer16BitAccess = VK_TRUE, .multiview = vkFeatures11_.multiview,
    // enable if supported
  ...
  VkPhysicalDeviceVulkan12Features deviceFeatures12 = {
    .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_2_FEATURES, .pNext = &deviceFeatures11,
    ...
    // enable if supported
    .descriptorIndexing = VK_TRUE, .shaderSampledImageArrayNonUniformIndexing = VK_TRUE,
    .descriptorBindingSampledImageUpdateAfterBind = VK_TRUE, .descriptorBindingStorageImageUpdateAfterBind = VK_TRUE,
    .descriptorBindingUpdateUnusedWhilePending = VK_TRUE, .descriptorBindingPartiallyBound = VK_TRUE,
    .descriptorBindingVariableDescriptorCount = VK_TRUE, .runtimeDescriptorArray = VK_TRUE,
    .scalarBlockLayout = VK_TRUE, .uniformBufferStandardLayout = VK_TRUE,
    .hostQueryReset = vkFeatures12_.hostQueryReset,
    // enable if supported
    .timelineSemaphore = VK_TRUE, .bufferDeviceAddress = VK_TRUE, .vulkanMemoryModel = vkFeatures12_.vulkanMemoryModel,
    // enable if supported
    .vulkanMemoryModelDeviceScope = vkFeatures12_.vulkanMemoryModelDeviceScope,
    // enable if supported
  };
  VkPhysicalDeviceVulkan13Features deviceFeatures13 = {
    .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_3_FEATURES, .pNext = &deviceFeatures12,
    .subgroupSizeControl = VK_TRUE, .synchronization2 = VK_TRUE, .dynamicRendering = VK_TRUE, .maintenance4 = VK_TRUE,
  };
#if defined(VK_API_VERSION_1_4)
  VkPhysicalDeviceVulkan14Features deviceFeatures14 = {
    .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_4_FEATURES, .pNext = &deviceFeatures13,
    .indexTypeUint8 = vkFeatures14_.indexTypeUint8,
    .dynamicRenderingLocalRead = vkFeatures14_.dynamicRenderingLocalRead,
  };
#endif // VK_API_VERSION_1_4
#if defined(VK_API_VERSION_1_4)
  void* createInfoNext = config_.vulkanVersion >= VulkanVersion_1_4
  ...
  VkPhysicalDeviceAccelerationStructureFeaturesKHR accelerationStructureFeatures = {
    .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ACCELERATION_STRUCTURE_FEATURES_KHR, .accelerationStructure = VK_TRUE,
    .accelerationStructureCaptureReplay = VK_FALSE, .accelerationStructureIndirectBuild = VK_FALSE,
    .accelerationStructureHostCommands = VK_FALSE, .descriptorBindingAccelerationStructureUpdateAfterBind = VK_TRUE,
  };
  VkPhysicalDeviceRayTracingPipelineFeaturesKHR rayTracingFeatures = {
    ...
    .rayTracingPipelineShaderGroupHandleCaptureReplayMixed = VK_FALSE, .rayTracingPipelineTraceRaysIndirect = VK_TRUE,
  ...
  VkPhysicalDeviceSwapchainMaintenance1FeaturesEXT swapchainMaintenance1Features = {
    .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SWAPCHAIN_MAINTENANCE_1_FEATURES_EXT, .swapchainMaintenance1 = VK_TRUE,
  };
  VkPhysicalDeviceIndexTypeUint8FeaturesEXT indexTypeUint8Features = {
    .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_INDEX_TYPE_UINT8_FEATURES_EXT, .indexTypeUint8 = VK_TRUE,
  };
    ...
    const char* name, bool& enabled, void* features = nullptr) mutable -> bool
  {
    ...
    if (features)
    {
      std::launder(reinterpret_cast<VkBaseOutStructure*>(features))->pNext = std::launder(
        reinterpret_cast<VkBaseOutStructure*>(createInfoNext));
  ...
  {
    if (!hasExtension(name1, allDeviceExtensions) || !hasExtension(name2, allDeviceExtensions)) return;
    enabled = true;
  ...
  VkPhysicalDeviceRobustness2FeaturesEXT robustness2Features = {
    .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ROBUSTNESS_2_FEATURES_EXT, .pNext = nullptr,
    .robustBufferAccess2 = vkRobustness2Features_.robustBufferAccess2,
    .robustImageAccess2 = vkRobustness2Features_.robustImageAccess2, .nullDescriptor = VK_TRUE
  };
...
#endif
  addOptionalExtensions(VK_KHR_ACCELERATION_STRUCTURE_EXTENSION_NAME, VK_KHR_DEFERRED_HOST_OPERATIONS_EXTENSION_NAME,
                        hasAccelerationStructure_, &accelerationStructureFeatures);
  ...
  addOptionalExtension(VK_EXT_SWAPCHAIN_MAINTENANCE_1_EXTENSION_NAME, has_EXT_swapchain_maintenance1_,
                       &vkSwapchainMaintenance1Features_);
  ...
  // check extensions
  {
    ...
    {
      if (!hasExtension(ext, allDeviceExtensions)) missingExtensions += "\n   " + std::string(ext);
    }
...
#undef CHECK_FEATURE_1_3
  ...
  const VkDeviceCreateInfo ci = {
    .sType = VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO, .pNext = createInfoNext, .queueCreateInfoCount = numQueues,
    .pQueueCreateInfos = ciQueue, .enabledExtensionCount = (uint32_t)deviceExtensionNames.size(),
    .ppEnabledExtensionNames = deviceExtensionNames.data(), .pEnabledFeatures = &deviceFeatures10,
  };
  ...
  VK_ASSERT(
    vk::setDebugObjectName(vkDevice_, VK_OBJECT_TYPE_DEVICE, (uint64_t)vkDevice_, "Device: VulkanContext::vkDevice_" ));
  VK_ASSERT(
    vk::setDebugObjectName(vkDevice_, VK_OBJECT_TYPE_INSTANCE, (uint64_t)vkInstance_,
      "Instance: VulkanContext::vkInstance_"));
  VK_ASSERT(
    vk::setDebugObjectName(vkDevice_, VK_OBJECT_TYPE_PHYSICAL_DEVICE, (uint64_t)vkPhysicalDevice_, (std::string(
      "Physical Device: VulkanContext::vkPhysicalDevice_ (") + deviceName + ")").c_str()));
  depthResolveMode_ = [this]() -> VkResolveModeFlagBits
  {
    ...
    if (modes & VK_RESOLVE_MODE_AVERAGE_BIT) return VK_RESOLVE_MODE_AVERAGE_BIT;
    // this mode is always available
    return VK_RESOLVE_MODE_SAMPLE_ZERO_BIT;
  }();
  immediate_ = std::make_unique<VulkanImmediateCommands>(vkDevice_, deviceQueues_.graphicsQueueFamilyIndex,
                                                         "VulkanContext::immediate_");
  // create Vulkan pipeline cache
  {
    const VkPipelineCacheCreateInfo cache_create_info = {
      VK_STRUCTURE_TYPE_PIPELINE_CACHE_CREATE_INFO, nullptr, VkPipelineCacheCreateFlags(0),
      config_.pipelineCacheDataSize, config_.pipelineCacheData,
    };
    vkCreatePipelineCache(vkDevice_, &cache_create_info, nullptr, &pipelineCache_);
  }
  pimpl_->vma_ = createVmaAllocator(vkPhysicalDevice_, vkDevice_, vkInstance_,
  ...
  createSampler({
                  .sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO, .pNext = nullptr, .flags = 0,
                  ...
                  .mipLodBias = 0.0f, .anisotropyEnable = VK_FALSE, .maxAnisotropy = 0.0f, .compareEnable = VK_FALSE,
                  .compareOp = VK_COMPARE_OP_ALWAYS, .minLod = 0.0f, .maxLod = static_cast<float>(MAX_MIP_LEVELS - 1),
                  .borderColor = VK_BORDER_COLOR_INT_OPAQUE_BLACK, .unnormalizedCoordinates = VK_FALSE,
                }, nullptr, Format::Invalid, "Sampler: default");
  ...
  else
  {
    ...
    setDebugObjectName(vkDevice_, VK_OBJECT_TYPE_COMMAND_POOL, (uint64_t)pimpl_->tracyCommandPool_,
                       "Command Pool: VulkanContextImpl::tracyCommandPool_");
    const VkCommandBufferAllocateInfo aiCommandBuffer = {
      .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO, .commandPool = pimpl_->tracyCommandPool_,
      .level = VK_COMMAND_BUFFER_LEVEL_PRIMARY, .commandBufferCount = 1,
    };
    VK_ASSERT(vkAllocateCommandBuffers(vkDevice_, &aiCommandBuffer, &pimpl_->tracyCommandBuffer_));
    if (hasCalibratedTimestamps_)
    {
      pimpl_->tracyVkCtx_ = TracyVkContextCalibrated(vkPhysicalDevice_, vkDevice_, deviceQueues_.graphicsQueue,
...
void vk::VulkanContext::createSurface(void* nativeWindow, void* display)
{
  ...
  const VkWin32SurfaceCreateInfoKHR ci = {
    .sType = VK_STRUCTURE_TYPE_WIN32_SURFACE_CREATE_INFO_KHR, .hinstance = GetModuleHandle(nullptr),
  ...
  const VkAndroidSurfaceCreateInfoKHR ci = {
    .sType = VK_STRUCTURE_TYPE_ANDROID_SURFACE_CREATE_INFO_KHR, .pNext = nullptr, .flags = 0,
  ...
  const VkXlibSurfaceCreateInfoKHR ci = {
    .sType = VK_STRUCTURE_TYPE_XLIB_SURFACE_CREATE_INFO_KHR, .flags = 0, .dpy = (Display*)display,
  ...
  const VkWaylandSurfaceCreateInfoKHR ci = {
    .sType = VK_STRUCTURE_TYPE_WAYLAND_SURFACE_CREATE_INFO_KHR, .flags = 0, .display = (wl_display*)display,
  ...
  const VkMacOSSurfaceCreateInfoMVK ci = {
    .sType = VK_STRUCTURE_TYPE_MACOS_SURFACE_CREATE_INFO_MVK, .flags = 0, .pView = nativeWindow,
...
#error Implement for other platforms
  ...
  if (vkPhysicalDevice_ != VK_NULL_HANDLE)
  {
    ...
    VK_ASSERT(vkGetPhysicalDeviceSurfaceSupportKHR(
      vkPhysicalDevice_, deviceQueues_.graphicsQueueFamilyIndex, vkSurface_, &supported ));
    if (!supported)
    {
...
vk::Result vk::VulkanContext::initSwapchain(uint32_t width, uint32_t height)
{
  ...
  else
  {
    ...
    VulkanSwapchain::InitInfo info = {
      .ctx = *this, .width = width, .height = height, .vSync = !config_.disableVSync, .oldSwapchain = VK_NULL_HANDLE
    };
    ...
    timelineSemaphore_ = createSemaphoreTimeline(vkDevice_, swapchain_->getNumSwapchainImages() - 1,
...
vk::Result vk::VulkanContext::growDescriptorPool(uint32_t maxTextures, uint32_t maxSamplers, uint32_t maxAccelStructs)
{
  ...
  {
    LOG_WARNING("Max Textures exceeded: {} (max {})", maxTextures,
  ...
  {
    LOG_WARNING("Max Samplers exceeded {} (max {})", maxSamplers,
  ...
  if (hasYUVImages)
  {
    ...
    for (const auto& obj : texturesPool_.objects_)
    {
      ...
      immutableSamplers.push_back(isYUVImage
                                    ? getOrCreateYcbcrSampler(vkFormatToFormat(img->vkImageFormat_))
                                    : dummySampler);
    }
  ...
  const VkDescriptorSetLayoutBinding bindings[kBinding_NumBindings] = {
    ...
    getDSLBinding(kBinding_YUVImages, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, (uint32_t)immutableSamplers.size(),
                  stageFlags, immutableSamplersData),
    getDSLBinding(kBinding_AccelerationStructures, VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR, maxAccelStructs,
                  stageFlags),
  };
  ...
  const VkDescriptorSetLayoutCreateInfo dslci = {
    .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO, .pNext = &setLayoutBindingFlagsCI,
  ...
  VK_ASSERT(
    vk::setDebugObjectName( vkDevice_, VK_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT, (uint64_t)vkDSL_,
      "Descriptor Set Layout: VulkanContext::vkDSL_"));
  {
    ...
    const VkDescriptorPoolCreateInfo ci = {
      .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO, .flags = VK_DESCRIPTOR_POOL_CREATE_UPDATE_AFTER_BIND_BIT,
      .maxSets = 1,
    ...
    const VkDescriptorSetAllocateInfo ai = {
      .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO, .descriptorPool = vkDPool_, .descriptorSetCount = 1,
      .pSetLayouts = &vkDSL_,
    };
...
vk::BufferHandle vk::VulkanContext::createBuffer(VkDeviceSize bufferSize, VkBufferUsageFlags usageFlags,
                                                 VkMemoryPropertyFlags memFlags, Result* outResult,
...
#undef ENSURE_BUFFER_SIZE
  ...
  const VkBufferCreateInfo ci = {
    .sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO, .pNext = nullptr, .flags = 0, .size = bufferSize,
    .usage = usageFlags, .sharingMode = VK_SHARING_MODE_EXCLUSIVE, .queueFamilyIndexCount = 0,
  ...
  if (usageFlags & VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT)
  {
    const VkBufferDeviceAddressInfo ai = {
      .sType = VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO, .buffer = buf.vkBuffer_,
    };
...
void vk::VulkanContext::bindDefaultDescriptorSets(VkCommandBuffer cmdBuf, VkPipelineBindPoint bindPoint,
...
void vk::VulkanContext::checkAndUpdateDescriptorSets()
{
  ...
  for (const auto& obj : texturesPool_.objects_)
  {
    ...
    infoSampledImages.push_back(VkDescriptorImageInfo{
      .sampler = VK_NULL_HANDLE, .imageView = isSampledImage ? view : dummyImageView,
      .imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
    });
    ASSERT(infoSampledImages.back().imageView != VK_NULL_HANDLE);
    infoStorageImages.push_back(VkDescriptorImageInfo{
      .sampler = VK_NULL_HANDLE, .imageView = isStorageImage ? storageView : dummyImageView,
    ...
    if (hasYcbcrSamplers)
    {
      ...
      infoYUVImages.push_back(VkDescriptorImageInfo{
        .imageView = isYUVImage ? view : dummyImageView, .imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
      });
    }
  }
  // 2. Samplers
  ...
  for (const auto& sampler : samplersPool_.objects_)
  {
    infoSamplers.push_back({
      .sampler = sampler.obj_ ? sampler.obj_ : samplersPool_.objects_[0].obj_, .imageView = VK_NULL_HANDLE,
  ...
  if (!handlesAccelStructs.empty())
  {
    ...
    write[numWrites++] = VkWriteDescriptorSet{
      .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, .pNext = &writeAccelStruct, .dstSet = vkDSet_,
      .dstBinding = kBinding_AccelerationStructures, .dstArrayElement = 0,
  ...
  {
    write[numWrites++] = VkWriteDescriptorSet{
      .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, .dstSet = vkDSet_, .dstBinding = kBinding_Textures,
      .dstArrayElement = 0, .descriptorCount = (uint32_t)infoSampledImages.size(),
      .descriptorType = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, .pImageInfo = infoSampledImages.data(),
    };
  }
  if (!infoSamplers.empty())
  {
    write[numWrites++] = VkWriteDescriptorSet{
      .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, .dstSet = vkDSet_, .dstBinding = kBinding_Samplers,
      .dstArrayElement = 0, .descriptorCount = (uint32_t)infoSamplers.size(),
      .descriptorType = VK_DESCRIPTOR_TYPE_SAMPLER, .pImageInfo = infoSamplers.data(),
    };
  }
  if (!infoStorageImages.empty())
  {
    write[numWrites++] = VkWriteDescriptorSet{
      .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, .dstSet = vkDSet_, .dstBinding = kBinding_StorageImages,
      .dstArrayElement = 0, .descriptorCount = (uint32_t)infoStorageImages.size(),
      .descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, .pImageInfo = infoStorageImages.data(),
    };
  }
  if (!infoYUVImages.empty())
  {
    write[numWrites++] = VkWriteDescriptorSet{
      .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, .dstSet = vkDSet_, .dstBinding = kBinding_YUVImages,
      .dstArrayElement = 0, .descriptorCount = (uint32_t)infoYUVImages.size(),
      .descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, .pImageInfo = infoYUVImages.data(),
    };
  }
...
vk::SamplerHandle vk::VulkanContext::createSampler(const VkSamplerCreateInfo& ci, [[maybe_unused]] Result* outResult,
                                                   Format yuvFormat, const char* debugName)
{
  PROFILER_FUNCTION_COLOR(PROFILER_COLOR_CREATE);
...
void vk::VulkanContext::querySurfaceCapabilities()
{
  ...
  const VkFormat depthFormats[] = {
    VK_FORMAT_D32_SFLOAT_S8_UINT, VK_FORMAT_D24_UNORM_S8_UINT, VK_FORMAT_D16_UNORM_S8_UINT, VK_FORMAT_D32_SFLOAT,
    VK_FORMAT_D16_UNORM
  };
