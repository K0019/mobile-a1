...
void OITSystem::SetupPasses(internal::RenderPassBuilder& passBuilder)
{
  ...
  vk::BufferDesc atomicCounterDesc{
    .usage = vk::BufferUsageBits_Storage, .storage = vk::StorageType::Device, .size = sizeof(uint32_t),
    .debugName = "Buffer: atomic counter"
  };
  // NEW: Packed OIT metadata buffer
  vk::BufferDesc oitBufferDesc{
    .usage = vk::BufferUsageBits_Storage, .storage = vk::StorageType::HostVisible, // Needs to be updated per frame
    .size = sizeof(OIT::OITBuffer), .debugName = "Buffer: OIT metadata"
  };
  CreateOITPipeline();
  // Register OIT Clear Pass - runs early to initialize OIT resources
  passBuilder.CreatePass().DeclareTransientResource(OITResources::FRAGMENT_BUFFER, fragmentBufferDesc).
              ...
              UseResource(OITResources::OIT_BUFFER, AccessType::Write).SetPriority(
                internal::RenderPassBuilder::PassPriority::EarlySetup).AddGenericPass(
                "OIT_Clear", [this](internal::ExecutionContext& context)
                {
                  ...
                  OIT::OITBuffer oitData = {
                    ...
                    .bufferTransparencyLists = context.GetvkContext().gpuAddress(
                      context.GetBuffer(OITResources::FRAGMENT_BUFFER)),
                    .texHeadsOIT = headTexture.index(), .maxOITFragments = maxFragments
                  };
                  vk::BufferHandle oit = context.GetBuffer(OITResources::OIT_BUFFER);
                  {
                    OIT::OITBuffer* mapped = reinterpret_cast<OIT::OITBuffer*>(context.GetvkContext().
                                                                                       getMappedPtr(oit));
  ...
  passBuilder.CreatePass().UseResource(RenderResources::SCENE_COLOR, AccessType::Write).
              UseResource(OITResources::HEAD_TEXTURE, AccessType::Read).
              UseResource(OITResources::FRAGMENT_BUFFER, AccessType::Read).
              SetPriority(internal::RenderPassBuilder::PassPriority::OITResolve).AddGraphicsPass(
                "OIT_Resolve", PassDeclarationInfo{
                  .colorAttachments = {
                    {
                      .textureName = RenderResources::SCENE_COLOR, .loadOp = vk::LoadOp::Load,
                      // Preserve opaque geometry
                ...
                }, [this](internal::ExecutionContext& context)
                {
                  ...
                  struct OITResolvePC
                  {
                    ...
                    uint32_t showHeatmap;
                      ...
                      .time = static_cast<float>(0), .opacityBoost = m_settings.opacityBoost,
...
void OITSystem::CreateOITPipeline()
{
  ...
  m_vertOIT = m_context.createShaderModule({codeVS, vk::ShaderStage::Vert, "Shader Module: OIT (vert)"});
  m_fragOIT = m_context.createShaderModule({codeFS, vk::ShaderStage::Frag, "Shader Module: OIT (frag)"});
  // Create OIT resolve pipeline - renders to SCENE_COLOR format
  m_pipelineOIT = m_context.createRenderPipeline({
    .smVert = m_vertOIT, .smFrag = m_fragOIT, .color = {{.format = m_context.getSwapchainFormat()}},
    // Will match SCENE_COLOR format
