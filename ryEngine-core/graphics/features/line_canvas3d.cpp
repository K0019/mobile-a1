...
void LineCanvas3D::plane(const vec3& o, const vec3& v1, const vec3& v2, int n1, int n2, float s1, float s2, const vec4& color, const vec4& outlineColor)
{
...
{
  vec3 pts[8] = {vec3(+size.x, +size.y, +size.z), vec3(+size.x, +size.y, -size.z), vec3(+size.x, -size.y, +size.z), vec3(+size.x, -size.y, -size.z), vec3(-size.x, +size.y, +size.z), vec3(-size.x, +size.y, -size.z), vec3(-size.x, -size.y, +size.z), vec3(-size.x, -size.y, -size.z),};
...
{
  const vec3 corners[] = {vec3(-1, -1, -1), vec3(+1, -1, -1), vec3(+1, +1, -1), vec3(-1, +1, -1), vec3(-1, -1, +1), vec3(+1, -1, +1), vec3(+1, +1, +1), vec3(-1, +1, +1)};
...
void LineCanvas3D::render(vk::IContext& ctx, const vk::Framebuffer& desc, vk::ICommandBuffer& buf, uint32_t numSamples)
{
  PROFILER_FUNCTION();
  ...
  {
    linesBuffer_[currentFrame_] = ctx.createBuffer({.usage = vk::BufferUsageBits_Storage, .storage = vk::StorageType::HostVisible, .size = requiredSize, .data = lines_.data()});
  ...
  if (pipeline_.empty() || pipelineSamples_ != numSamples)
  {
    ...
    vert_ = ctx.createShaderModule({codeVS, vk::ShaderStage::Vert, "Shader Module: 3d_line_canvas (vert)"});
    frag_ = ctx.createShaderModule({codeFS, vk::ShaderStage::Frag, "Shader Module: 3d_line_canvas (frag)"});
    pipeline_ = ctx.createRenderPipeline({.topology = vk::Topology::Line, .smVert = vert_, .smFrag = frag_, .color = {{.format = ctx.getFormat(desc.color[0].texture), .blendEnabled = true, .srcRGBBlendFactor = vk::BlendFactor::SrcAlpha, .dstRGBBlendFactor = vk::BlendFactor::OneMinusSrcAlpha,}}, .depthFormat = desc.depthStencil.texture ? ctx.getFormat(desc.depthStencil.texture) : vk::Format::Invalid, .cullMode = vk::CullMode::None, .samplesCount = numSamples,}, nullptr);
  }
  ...
  currentFrame_ = (currentFrame_ + 1) % std::size(linesBuffer_);
}
