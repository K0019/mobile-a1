...
namespace
{
  ...
  const Resource::ProcessedMorphChannel* findMorphChannel(const Resource::AnimationClip& clip, uint32_t meshIndex,
                                                          const std::string& meshName, const std::string& objectName)
  {
    ...
    {
      if (channel.meshIndex != UINT32_MAX && channel.meshIndex == meshIndex) return &channel;
    }
    // Fallback to mesh name
    for (const auto& channel : channels)
    {
      if (!channel.meshName.empty() && channel.meshName == meshName) return &channel;
    }
    // Last resort: try matching against scene object name
    for (const auto& channel : channels)
    {
      if (channel.meshName == objectName) return &channel;
    }
    return nullptr;
  }

  void applyMorphKey(const Resource::ProcessedMorphKey& key, float scale, std::vector<float>& weights)
  {
    if (scale <= 0.0f) return;
    for (size_t i = 0; i < key.targetIndices.size(); ++i)
    {
      const uint32_t target = key.targetIndices[i];
      if (target < weights.size()) weights[target] += key.weights[i] * scale;
    }
  }

  void accumulateMorphWeights(const Resource::AnimationClip& clip, const SceneObject& object, float time,
                              float clipWeight, std::vector<float>& weights, const Resource::ResourceManager& rm)
  {
    if (clipWeight <= 0.0f || weights.empty()) return;
    ...
    const auto* channel = findMorphChannel(clip, object.getSourceMeshIndex(), meshName, object.name);
    if (!channel || channel->keys.empty()) return;
    ...
    size_t keyIndex = 0;
    while (keyIndex + 1 < channel->keys.size() && time > channel->keys[keyIndex + 1].time)
    {
      ++keyIndex;
    }
    ...
    const float factor = span > 0.0f ? std::clamp((time - keyA.time) / span, 0.0f, 1.0f) : 0.0f;
...
namespace Animation
{
  void Animate(const Resource::ResourceManager& rm, SceneObject& obj, float dt)
  {
    if (obj.type != SceneObjectType::Mesh || !obj.mesh.isValid() || !obj.anim) return;
    SceneObject::AnimBinding& anim = *obj.anim;
    if ((anim.flags & SceneObject::AnimBinding::Playing) == 0) return;
    const bool crossfade = (anim.flags & SceneObject::AnimBinding::Crossfade) != 0;
    const Resource::AnimationClip* clipA = anim.clipA != Resource::INVALID_CLIP_ID ? &rm.Clip(anim.clipA) : nullptr;
    ...
    if (clipA)
    {
      anim.timeA = advanceTime(anim.timeA, dt * anim.speed, clipA->duration(),
                               (anim.flags & SceneObject::AnimBinding::Loop) != 0);
    }
    if (clipB)
    {
      anim.timeB = advanceTime(anim.timeB, dt * anim.speed, clipB->duration(),
                               (anim.flags & SceneObject::AnimBinding::Loop) != 0);
    }
    // --- Skinning Update ---
    if (anim.jointCount > 0)
    {
      ...
      if (anim.skinMatrices.size() != anim.jointCount) anim.skinMatrices.resize(anim.jointCount);
      ...
      else
      {
        ...
        for (const uint32_t jSkel : topoOrder)
        {
          ...
          auto pose = (channelIdxA >= 0) ? clipA->sampleChannel(channelIdxA, anim.timeA) : skeleton.bindPoseTRS(jSkel);
          if (clipB && blend > 0.0f)
          {
          ...
          globalsSkel[jSkel] = (parentIdx >= 0) ? globalsSkel[parentIdx] * local : local;
        }
    ...
    {
      if (anim.morphWeights.size() != anim.morphCount) anim.morphWeights.assign(anim.morphCount, 0.0f);
      ...
      if (clipA)
      {
        ...
        if (clipB && blend > 0.0f) accumulateMorphWeights(*clipB, obj, anim.timeB, blend, anim.morphWeights, rm);
      }
    }
  }
} // namespace Animation
