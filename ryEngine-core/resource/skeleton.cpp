...
namespace Resource
{
  ...
  Skeleton::Skeleton(const ProcessedSkeleton& skeleton)
    : m_parentIndices(skeleton.parentIndices),
      m_inverseBindMatrices(skeleton.inverseBindMatrices),
      m_bindPoseMatrices(skeleton.bindPoseMatrices),
      m_jointNames(skeleton.jointNames)
  {
  ...
  {
    if(joint >= m_inverseBindMatrices.size())
  ...
  {
    if(joint >= m_bindPoseMatrices.size())
      return identityMatrix();
  ...
  {
      if (joint >= m_decomposedBindPose.size())
          return identityJointSample();
