...
template<typename T>
concept App = requires(T& app, Context& ctx,FrameData& data) {
    { app.Initialize(ctx) } -> std::same_as<void>;
    { app.Update(ctx, data) } -> std::same_as<void>;
    { app.Shutdown(ctx) } -> std::same_as<void>;
};
...
void Engine<AppType>::Initialize()
{
    ...
    Core::Lifecycle().onStateChange =
        [this](Core::AppState old, Core::AppState newState) {
            OnLifecycleChange(old, newState);
        };

    Core::Lifecycle().onLowMemory =
        [this]() {
            OnLowMemory();
        };

    Core::Display().onResize =
