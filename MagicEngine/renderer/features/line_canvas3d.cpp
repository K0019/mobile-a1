...
void LineCanvas3D::plane(const vec3& o, const vec3& v1, const vec3& v2, int n1, int n2, float s1, float s2,
...
{
  vec3 pts[8] = {
    vec3(+size.x, +size.y, +size.z), vec3(+size.x, +size.y, -size.z), vec3(+size.x, -size.y, +size.z),
    vec3(+size.x, -size.y, -size.z), vec3(-size.x, +size.y, +size.z), vec3(-size.x, +size.y, -size.z),
...
void LineCanvas3D::frustum(const mat4& camView, const mat4& camProj, const vec4& color)
{
  ...
  // bottom
  {
    ...
    for (int i = 0; i != gridLines; i++, p1 += s1, p2 += s2) line(p1, p2, gridColor);
  }
  // top
  {
    vec3 p1 = pp[2];
...
void LineCanvas3D::render(vk::IContext& ctx, const vk::Framebuffer& desc, vk::ICommandBuffer& buf, uint32_t numSamples)
{
  ...
  if (currentBufferSize_[currentFrame_] < requiredSize)
  {
    linesBuffer_[currentFrame_] = ctx.createBuffer({
      .usage = vk::BufferUsageBits_Storage, .storage = vk::StorageType::HostVisible, .size = requiredSize,
      .data = lines_.data()
    });
  ...
  if (pipeline_.empty() || pipelineSamples_ != numSamples)
  {
    ...
    pipeline_ = ctx.createRenderPipeline({
                                           .topology = vk::Topology::Line, .smVert = vert_, .smFrag = frag_,
                                           .color = {
                                             {
                                               .format = ctx.getFormat(desc.color[0].texture), .blendEnabled = true,
