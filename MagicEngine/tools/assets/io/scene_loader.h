...
namespace Resource
{
  ...
  public:
    ...
    SceneLoadResult loadScene(const std::filesystem::path& path,
                              const LoadingConfig& config = LoadingConfig::createBalanced(),
  ...
  private:
    ...
    void extractObjectsFromNode(const aiNode* node, const mat4& parentTransform, const aiScene* scene,
                                std::vector<SceneObject>& objects) const;

    void resolveSceneObjectHandles(std::vector<SceneObject>& objects, const std::vector<MeshHandle>& meshHandles,
