...
namespace Resource
{
  class MeshOptimizer
  {
    public:
      ...
      static OptimizationResult optimize(std::vector<Vertex>& vertices, std::vector<uint32_t>& indices, bool generateTangents = false);

      static bool generateTangents(std::vector<Vertex>& vertices, std::vector<uint32_t>& indices);
    ...
    private:
      ...
      static std::vector<Vertex> unweldMesh(const std::vector<Vertex>& vertices, const std::vector<uint32_t>& indices);

      static void generateTangentsUnwelded(std::vector<Vertex>& unweldedVertices);

      static void reindexMesh(std::vector<Vertex>& vertices, std::vector<uint32_t>& indices, const std::vector<Vertex>& unweldedVertices);
